...
const WEB_CATEGORY_PRESETS={
  broad:{label:'Весь интернет',label_en:'Entire web',search_scope:'internet',allowed_domains:[],excluded_domains:[],news_interests:[],prefer_russian:true},
  marketplaces:{label:'Маркетплейсы',label_en:'Marketplaces',search_scope:'selected',allowed_domains:['wildberries.ru','ozon.ru','market.yandex.ru','aliexpress.ru','lamoda.ru','megamarket.ru'],excluded_domains:[],news_interests:[],prefer_russian:true},
  registries:{label:'Госреестры',label_en:'Government registries',search_scope:'selected',allowed_domains:['zakupki.gov.ru','egrul.nalog.ru','egrip.nalog.ru','rosreestr.gov.ru','fssp.gov.ru','nalog.gov.ru'],excluded_domains:[],news_interests:[],prefer_russian:true},
  list:{label:'Сайты по списку',label_en:'Sites by list',search_scope:'selected',allowed_domains:[],excluded_domains:[],news_interests:[],prefer_russian:true},
};
const ARTIFACT_FORMATS=['txt','md','json','csv','pdf','docx','xlsx','pptx'];
...
function normalizeDomainLines(values){return Array.isArray(values)?values.map(value=>String(value||'').toLowerCase().trim().replace(/^https?:\/\//,'').replace(/\/.*$/,'').replace(/^\./,'')).filter(Boolean):[]}
function sameDomainList(left,right){const a=normalizeDomainLines(left),b=normalizeDomainLines(right);return a.length===b.length&&a.every((item,index)=>item===b[index])}
function webPresetFromPreferences(preferences){
  ...
  const interests=Array.isArray(preferences?.news_interests)?preferences.news_interests.map(item=>String(item||'').trim()).filter(Boolean):[];
  ...
  for(const [key,preset] of Object.entries(WEB_CATEGORY_PRESETS)){
    ...
    if(JSON.stringify(interests)!==JSON.stringify(preset.news_interests))continue;
    if(Boolean(preferRussian)!==Boolean(preset.prefer_russian))continue;
    return key;
  }
  return scope==='selected'?'list':'broad';
}
function currentWebPreferences(){return {search_scope:$('#webSearchScope')?.value||state.webPreferences?.search_scope||'internet',allowed_domains:linesToDomains($('#webAllowedDomains')?.value||''),excluded_domains:linesToDomains($('#webExcludedDomains')?.value||''),news_interests:String($('#webNewsInterests')?.value||'').split(/\r?\n|,/).map(item=>item.trim()).filter(Boolean),prefer_russian:$('#webPreferRussian')?$('#webPreferRussian').checked:state.webPreferences?.prefer_russian!==false}}
function renderWebPresetButtons(){const current=webPresetFromPreferences(currentWebPreferences());$$('[data-web-preset]').forEach(button=>button.classList.toggle('active',button.dataset.webPreset===current))}
function applyWebPreset(key){const preset=WEB_CATEGORY_PRESETS[key];if(!preset)return;if($('#webSearchScope'))$('#webSearchScope').value=preset.search_scope;if($('#webAllowedDomains'))$('#webAllowedDomains').value=(preset.allowed_domains||[]).join('\n');if($('#webExcludedDomains'))$('#webExcludedDomains').value=(preset.excluded_domains||[]).join('\n');if($('#webNewsInterests'))$('#webNewsInterests').value=(preset.news_interests||[]).join('\n');if($('#webPreferRussian'))$('#webPreferRussian').checked=Boolean(preset.prefer_russian);renderWebPresetButtons();toast(`${L('Категория поиска','Search category')}: ${langKey()==='en'?(preset.label_en||preset.label):preset.label}`,'success')}
function applyArtifactFormat(format,updateName=true){const field=$('#artifactFormat');const nameField=$('#artifactName');if(field)field.value=format;if(updateName&&nameField){nameField.value=(nameField.value||'document').replace(/\.[^.]+$/, '')+`.${format}`}$$('[data-artifact-preset]').forEach(button=>button.classList.toggle('active',button.dataset.artifactPreset===format))}
function saveStore(){
...
function relativeTime(timestamp){
  ...
  if(sameDay)return date.toLocaleTimeString(langKey()==='en'?'en-US':'ru-RU',{hour:'2-digit',minute:'2-digit'});
  return date.toLocaleDateString(langKey()==='en'?'en-US':'ru-RU',{day:'2-digit',month:'short'});
}
...
function capabilityStatusText(capability,entitled){
  ...
  if(!entitled)return L('Недоступно на текущем тарифе','Unavailable on the current plan');
  if(status==='ready')return L('Доступно в текущей сборке','Available in this build');
  if(status==='admin')return L('Доступно через администратора','Available through administration');
  if(status==='degraded'||status==='unavailable')return L('Доступно с ограничениями текущего окружения','Limited by the current environment');
  return L('Подключается отдельным продуктовым слоем','Coming in a separate product layer');
}
function capabilityBadgeText(capability,entitled){
  ...
  if(status==='degraded'||status==='unavailable')return L('Ограничено','Limited');
...
function sourceKindLabel(kind){
  const labels={news:["Новость","News"],product:["Товар","Product"],real_estate:["Объект","Property"],procurement:["Закупка","Procurement"],marketplace:["Маркетплейс","Marketplace"],registry:["Реестр","Registry"],source:["Источник","Source"]};
...
function renderSources(message){
  ...
  const title=node('div','sources-title',`${mainKind==='news'?L('Найденные новости','News found'):mainKind==='product'?L('Найденные варианты','Options found'):mainKind==='real_estate'?L('Найденные объекты','Properties found'):mainKind==='procurement'?L('Найденные закупки','Procurements found'):mainKind==='marketplace'?L('Найденные товары','Products found'):mainKind==='registry'?L('Найденные записи','Records found'):L('Источники','Sources')} · ${sources.length}`);
  heading.append(title);wrap.append(heading);
  const list=node('div','source-list result-card-grid');
  for(const [index,source] of sources.entries()){
    const kind=['news','product','real_estate','procurement','marketplace','registry'].includes(source.kind)?source.kind:'source';
...
async function uploadSelectedFiles(files){
  ...
  if(selected.length>6){
    toast(L('За один раз можно загрузить не больше 6 файлов','No more than 6 files can be uploaded at once'),'error');
    return;
  }
...
function renderCapabilities(){
  ...
  for(const [key,capability] of Object.entries(state.system?.capabilities||{})){
    ...
    const copy=node('div','');copy.append(node('strong','',capability.label||key),node('span','',capabilityStatusText(capability,entitled)));
    const statusClass=!entitled?'locked':capabilityVisualStatus(capability.status);row.append(copy,node('span',`capability-badge ${statusClass}`,capabilityBadgeText(capability,entitled)));host.append(row);
  }
}
...
function runtimeProfile(){return String(state.system?.runtime_profile||'local').toLowerCase()==='server'?'server':'local'}
function editionLabel(lang=langKey()){return runtimeProfile()==='server'?(lang==='en'?'Server edition':'Серверная версия'):(lang==='en'?'Local edition':'Локальная версия')}
function defaultChatTitle(){return L('Новый чат','New chat')}
const THEME_COLOR_MAP={system:'#0b0c0f',dark:'#0b0c0f',light:'#f6f7f9',ocean:'#0d1530',forest:'#0d180f',sunset:'#1e120c',sand:'#1b160d',coral:'#1f1015'};
function effectiveTheme(value){const theme=String(value||'system');if(theme==='system')return matchMedia('(prefers-color-scheme: light)').matches?'light':'dark';return theme}
function applyTheme(value){const theme=effectiveTheme(value||state.experiencePreferences?.theme||'system');document.documentElement.dataset.theme=theme;localStorage.setItem('par-theme-preference',value||'system');const root=document.documentElement;const palette={light:{bg:'#f6f7f9',panel:'#ffffff',panel2:'#f0f2f5',panel3:'#e7eaf0',line:'#dce0e7',line2:'#cdd3dc',muted:'#667085',text:'#16181d',soft:'#414753',accent:'#17191f'},dark:{bg:'#0b0c0f',panel:'#111318',panel2:'#161920',panel3:'#1d212a',line:'#272b35',line2:'#343945',muted:'#9096a3',text:'#f7f7f8',soft:'#c8ccd4',accent:'#f3f4f6'},ocean:{bg:'#08111d',panel:'#0e1727',panel2:'#132033',panel3:'#19304a',line:'#20344f',line2:'#2c4d77',muted:'#8eaad1',text:'#f3f8ff',soft:'#c8ddff',accent:'#76b7ff'},forest:{bg:'#08150e',panel:'#0e1c12',panel2:'#13261a',panel3:'#183524',line:'#23402c',line2:'#35604a',muted:'#8fbea0',text:'#f1fbf5',soft:'#cce8d4',accent:'#75d29f'},sunset:{bg:'#170f0c',panel:'#241713',panel2:'#2d1d18',panel3:'#39251f',line:'#4d2c24',line2:'#6a4035',muted:'#d2ab97',text:'#fff8f4',soft:'#f0d5c8',accent:'#ff9c66'},sand:{bg:'#17130c',panel:'#231d13',panel2:'#2c2418',panel3:'#382e1f',line:'#4a3d28',line2:'#685739',muted:'#d7c099',text:'#fffaf1',soft:'#f1e0c2',accent:'#e7c78c'},coral:{bg:'#171012',panel:'#241518',panel2:'#2e1b20',panel3:'#3a2329',line:'#4d2a34',line2:'#6f3b4a',muted:'#d9a8b8',text:'#fff7fa',soft:'#f4d0dc',accent:'#ff7e9b'}}[theme]||null;if(palette){for(const [key,val] of Object.entries(palette))root.style.setProperty(`--${key}`,val)}document.querySelector('meta[name="theme-color"]')?.setAttribute('content',THEME_COLOR_MAP[theme]||THEME_COLOR_MAP.dark)}
...
function renderExperiencePreferences(){const p=state.experiencePreferences||{};if($('#uiLanguage'))$('#uiLanguage').value=p.ui_language||'ru';if($('#responseLanguage'))$('#responseLanguage').value=p.response_language||'auto';if($('#themeSelect')){$('#themeSelect').value=p.theme||'system';const themeKeys={ocean:'theme_ocean',forest:'theme_forest',sunset:'theme_sunset',sand:'theme_sand',coral:'theme_coral'};for(const option of $('#themeSelect').options){const key=themeKeys[option.value];const locked=!!key&&!entitlementEnabled(key);option.disabled=locked;option.title=locked?L('Доступно на более высоком тарифе','Available on a higher plan'):''}}if($('#uiScale'))$('#uiScale').value=p.ui_scale||'normal';if($('#executionPolicy'))$('#executionPolicy').value=p.execution_policy||'auto';if($('#tonePreset'))$('#tonePreset').value=p.tone||'normal';if($('#profileNotes'))$('#profileNotes').value=p.profile_notes||'';if($('#executionQuickLabel'))$('#executionQuickLabel').textContent=executionLabel(p.execution_policy||'auto');applyTheme(p.theme||'system');applyUiScale(p.ui_scale||'normal');applyLanguage(p.ui_language||'ru')}
async function loadExperiencePreferences(){try{const payload=await api('/api/preferences/experience');state.experiencePreferences=payload.preferences||{};renderExperiencePreferences()}catch(_){state.experiencePreferences={ui_language:localStorage.getItem('par-ui-language')||'ru',response_language:'auto',theme:localStorage.getItem('par-theme-preference')||'system',execution_policy:'auto',tone:'normal',ui_scale:localStorage.getItem('par-ui-scale')||'normal',profile_notes:''};renderExperiencePreferences()}}
async function saveExperiencePreferences(){const out=$('#experienceState');try{const payload=await api('/api/preferences/experience',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ui_language:$('#uiLanguage').value,response_language:$('#responseLanguage').value,theme:$('#themeSelect').value,ui_scale:$('#uiScale').value,execution_policy:$('#executionPolicy').value,tone:$('#tonePreset').value,profile_notes:$('#profileNotes')?.value||''})});state.experiencePreferences=payload.preferences;renderExperiencePreferences();out.textContent=tr('saved');out.className='job-state completed';toast(langKey()==='en'?'Settings saved':'Настройки сохранены','success')}catch(error){out.textContent=friendlyError(error).title;out.className='job-state failed'}}
...
async function init(){
  ...
  try{
    ...
    const account=$('#accountEntry');if(account){if(state.auth?.user){account.href='/account';account.querySelector('.account-label').textContent=state.auth.user.display_name||L('Аккаунт','Account')}else if(state.system?.auth?.mode==='accounts'){account.href='/login';account.querySelector('.account-label').textContent=L('Войти','Sign in')}else{account.href='/account';account.querySelector('.account-label').textContent=L('Локальный профиль','Local profile')}}
    const role=String(state.auth?.user?.role||'').toUpperCase();const roleAdmin=['OWNER','ADMIN'].includes(role);const personalOwner=state.system?.auth?.mode==='personal';$('#adminEntry').hidden=personalOwner||!roleAdmin;const adminSettings=$('#adminSettingsLink');if(adminSettings)adminSettings.hidden=!(roleAdmin||personalOwner);const logout=$('#logoutEntry');if(logout)logout.hidden=!(state.system?.auth?.mode==='accounts'&&state.auth?.user);
    ...
    try{const prefPayload=await api('/api/preferences/web');state.webPreferences=prefPayload.preferences||null;renderWebPreferences()}catch(_){state.webPreferences=null;renderWebPreferences()}
...
function setHealth(ok,text){for(const selector of ['#dot','#sideDot'])$(selector).className=`dot ${ok?'ok':'bad'}`;$('#health').textContent=text;$('#sideHealth').textContent=text;const subtitle=document.querySelector('.sidebar-status small');if(subtitle)subtitle.textContent=runtimeProfile()==='server'?tr('serverStatus'):tr('localStatus')}
...
async function signOut(){try{await api('/api/auth/logout',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'})}catch(_){}location.href='/login'}
async function openArchive(){state.activeFolderId='__archived__';const first=state.conversations.find(c=>c.archived_at);if(first)await loadConversation(first.id,false);saveStore();renderAll();closeSidebar()}
...
async function createFolder(){const name=await promptAction(L('Новый проект','New project'),L('Проекты помогают группировать связанные диалоги.','Projects help group related chats.'),L('Новый проект','New project'),L('Название проекта','Project name'));if(name===null||!String(name).trim())return;const result=await api('/api/folders',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name})});state.folders.push(result.folder);state.activeFolderId=result.folder.id;const active=current();if(active&&active.messages?.length&&active.folder_id!==result.folder.id){const moveNow=await confirmAction(L('Перенести текущий диалог в новый проект?','Move the current chat to the new project?'),L(`Проект «${result.folder.name}» уже создан. Можно сразу перенести туда текущий диалог.`,`Project “${result.folder.name}” was created. You can move the current chat there now.`),L('Перенести','Move'));if(moveNow){await api(`/api/conversations/${active.id}/move`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({folder_id:result.folder.id})});await loadServerStore(state.search);toast(L('Проект создан, диалог перенесён','Project created and chat moved'),'success');return}}saveStore();renderAll();toast(L('Проект создан. Новый чат будет открываться внутри него.','Project created. New chats will now open inside it.'),'success')}
...
function tourState(){return localStorage.getItem(TOUR_STATE_KEY)||''}
function setTourState(status){if(status)localStorage.setItem(TOUR_STATE_KEY,status);else localStorage.removeItem(TOUR_STATE_KEY)}
function consumeTourTrigger(){const armed=sessionStorage.getItem(TOUR_TRIGGER_KEY)==='1';if(armed)sessionStorage.removeItem(TOUR_TRIGGER_KEY);return armed}
...
function renderHelpCapabilities(){const host=$('#helpCapabilities');host.replaceChildren();for(const [key,item] of Object.entries(state.system?.capabilities||{})){const row=node('div','help-capability');const status=String(item.status||'').toLowerCase();row.append(node('span','',status==='ready'?'✓':status==='admin'?'⚙':status==='degraded'?'!':'○'));const copy=node('div','');const detail=status==='ready'?L('Доступно сейчас','Available now'):status==='degraded'?L('Ограниченно доступно','Limited availability'):status==='admin'?L('Настраивается администратором','Configured by an administrator'):status==='planned'?L('Запланировано, но ещё не подключено','Planned but not connected yet'):L('Сейчас недоступно','Currently unavailable');copy.append(node('strong','',item.label||key),node('small','',detail));row.append(copy);host.append(row)}host.hidden=false}
...
async function startTour(force=false){closeHelp();if(!force){const local=tourState();if(['completed','skipped'].includes(local))return;try{const value=await api('/api/onboarding');const remote=value.state?.status;if(['completed','skipped'].includes(remote)){setTourState(remote);return}}catch(_){if(['completed','skipped'].includes(local))return}}tourIndex=0;$('#tourLayer').hidden=false;setTourState('in_progress');await persistTour('in_progress');renderTour()}
async function finishTour(status='completed'){setTourState(status);await persistTour(status);$('#tourLayer').hidden=true;input.focus()}
async function maybeStartTour(){if(!consumeTourTrigger())return;if(['completed','skipped'].includes(tourState()))return;await startTour(false)}

function bindEvents(){
  $('#runtimeRetry').onclick=()=>health();$('#newChat').onclick=()=>newConversation(true);$('#newFolder').onclick=createFolder;$('#brandHelp').onclick=openHelp;$('#archiveEntry').onclick=()=>openArchive().catch(error=>toast(error.message,'warning'));$('#logoutEntry').onclick=signOut;$('#helpEntry').onclick=openHelp;$('#feedbackEntry').onclick=()=>{openHelp();const box=$('#feedbackInline');if(box)box.hidden=false;setTimeout(()=>$('#feedbackMessage')?.focus(),0)};$('#closeHelp').onclick=closeHelp;$('#helpBackdrop').onclick=e=>{if(e.target===$('#helpBackdrop'))closeHelp()};$('#restartTour').onclick=()=>startTour(true);$('#showCapabilities').onclick=renderHelpCapabilities;$('#collapseSidebar').onclick=()=>setSidebarCollapsed(!$('#sidebar').classList.contains('collapsed'));bindSidebarResize();$('#openSidebar').onclick=openSidebar;$('#closeSidebar').onclick=closeSidebar;$('#sidebarBackdrop').onclick=closeSidebar;
  ...
  $('#fileInput').onchange=async event=>{await uploadSelectedFiles(event.target.files);event.target.value=''};$('#uploadArtifact').onclick=openFilePicker;$('#createArtifact').onclick=createArtifactFromUi;$('#artifactFormat').onchange=event=>{applyArtifactFormat(event.target.value)};
