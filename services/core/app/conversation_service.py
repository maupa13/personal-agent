    ...
    def export_all(self, user_id: str, *, limit: int = 500) -> dict[str, Any]:
        ...
        with self._lock, self._db() as conn:
            ...
            if ids:
                ...
                rows = conn.execute(
                    f"SELECT id,conversation_id,role,content,kind,sources_json,attachments_json,metadata_json,created_at,updated_at FROM messages WHERE user_id=? AND conversation_id IN ({placeholders}) ORDER BY created_at ASC,id ASC",
                    [user_id, *ids],
                ).fetchall()
                for row in rows:
                    item = dict(row)
    ...
    def get(self, user_id: str, conversation_id: str) -> dict[str, Any]:
        with self._lock, self._db() as conn:
            ...
            for m in conn.execute(
                "SELECT id,role,content,kind,sources_json,attachments_json,metadata_json,created_at,updated_at FROM messages WHERE conversation_id=? AND user_id=? ORDER BY created_at ASC,id ASC",
                (conversation_id, user_id),
            ):
                item = dict(m)
                try:
    ...
    def add_message(
        ...
        *,
        ...
        with self._lock, self._db() as conn:
            ...
            conn.execute(
                "INSERT INTO messages(id,conversation_id,user_id,role,content,kind,sources_json,attachments_json,metadata_json,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)",
                (mid, conversation_id, user_id, role, content, str(kind or "message")[:40], json.dumps(sources, ensure_ascii=False), json.dumps(attachments, ensure_ascii=False), json.dumps(metadata, ensure_ascii=False), ts, ts),
            )
            ...
            conn.commit()
        return {"id": mid, "role": role, "content": content, "kind": kind, "sources": sources, "attachments": attachments, "metadata": metadata, "created_at": ts, "updated_at": ts}
        ...
        for raw in conversations[:100]:
            try:
                ...
                for msg in list(raw.get("messages") or [])[-200:]:
                    ...
                    self.add_message(user_id, conv["id"], role=role, content=str(msg.get("content", "")), kind=str(msg.get("kind", "message")), sources=list(msg.get("sources") or []), attachments=list(msg.get("attachments") or []), metadata=dict(msg.get("metadata") or {}))
                imported += 1
            except Exception:
                skipped += 1
