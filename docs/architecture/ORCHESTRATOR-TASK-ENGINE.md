# Orchestrator / Task Engine — v0.7.2
...
v0.7.2 proves the durable task foundation with one real multi-capability workflow. It does **not** claim the whole future planner/tool DAG is complete. Future task types must use the same persisted step/event/verification contract and add mandatory USER E2E before becoming `ready`.
