...
## Исправление 16–17.09.2026

Selective OpenAI egress теперь включён в Core: [конфигурация](../deploy/egress/README.md).
Не подключайте старый внешний `compose.egress.yaml`, подменяющий main.py.
В `.env.vps` сохраните `PA_LLM_EGRESS_PROXY_URL` и задайте случайный
`PA_SEARXNG_SECRET`. Основной Compose поднимает поиск, browser и code-worker.
Доверенный reverse proxy имеет адрес `172.30.0.254`; Core принимает client IP
из X-Forwarded-For только от адресов `PA_TRUSTED_PROXY_IPS`.

Проверки и ограничения: [матрица продукта](PRODUCT-REPAIR-2026-09-16.md).
