...
Selective OpenAI egress ?????? ??????? ? Core: [????????????](../deploy/egress/README.md).
?? ??????????? ?????? ??????? `compose.egress.yaml`, ??????????? main.py.
? `.env.vps` ????????? `PA_LLM_EGRESS_PROXY_URL` ? ??????? ?????????
`PA_SEARXNG_SECRET`. ???????? Compose ????????? ?????, browser ? code-worker.
?????????? reverse proxy ????? ????? `172.30.0.254`; Core ????????? client IP
?? X-Forwarded-For ?????? ?? ??????? `PA_TRUSTED_PROXY_IPS`.

???????? ? ???????????: [??????? ????????](PRODUCT-REPAIR-2026-09-16.md).
