...
def server_bundle(version: str, profile: str, domain: str, admin_token: str, registration_policy: str = "open") -> bytes:
    ...
    payloads = {"compose.yaml": compose.encode(), ".env.server": env.encode(), "Caddyfile": caddy.encode(), "README.txt": readme.encode()}
...
def deploy(session: SSHSession, bundle: bytes, version: str, *, remote_root: str = "/opt/personal-agent") -> dict[str, Any]:
    ...
    verify_cmd = (
        ...
        "if docker compose --env-file .env.server -f compose.yaml exec -T core python -c \"import urllib.request,json; x=json.load(urllib.request.urlopen('http://127.0.0.1:8080/api/health',timeout=3)); assert x.get('ready')\" >/dev/null 2>&1; then exit 0; fi; sleep 2; done; exit 1"
    )
...
def rollback(session: SSHSession, *, remote_root: str = "/opt/personal-agent") -> dict[str, Any]:
    cmd = (
        ...
        "cd current; docker compose --env-file .env.server -f compose.yaml up -d --build"
    )
