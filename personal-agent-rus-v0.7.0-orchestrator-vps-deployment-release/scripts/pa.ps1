...
function Ensure-Env {
  $defaults = [ordered]@{
    ...
    'PA_CORE_IMAGE'='personal-agent-core:0.7.0'
    'PA_BROWSER_IMAGE'='personal-agent-browser:0.7.0'
    'PA_SEARXNG_IMAGE'='searxng/searxng:2026.8.5-1689cb1b5'
    'PA_CODE_WORKER_IMAGE'='personal-agent-code-worker:0.7.0'
  ...
  if(Test-Path -LiteralPath $EnvFile){
    ...
    if(([string]$values['PA_CORE_IMAGE']) -match '^(rus-personal-agent-core|personal-agent-core):'){ $values['PA_CORE_IMAGE']='personal-agent-core:0.7.0';$changed=$true }
    if($values.Contains('PA_BROWSER_IMAGE') -and ([string]$values['PA_BROWSER_IMAGE']) -match '^personal-agent-browser:'){ $values['PA_BROWSER_IMAGE']='personal-agent-browser:0.7.0';$changed=$true }
    if($changed){
...
function Test-DeterministicBrowserSecurity([string]$ArtifactsRoot) {
  $bootstrap=Get-EnvValue 'PA_BOOTSTRAP_MODEL';if(-not $bootstrap){$bootstrap='qwen3:0.6b'}
  $coreImage=Get-EnvValue 'PA_CORE_IMAGE';if(-not $coreImage){$coreImage='personal-agent-core:0.7.0'}
  $tests=Join-Path $Root 'tests'
...
function Test-ReferenceReleaseAcceptance {
  ...
  Invoke-Compose -ComposeArgs @('restart','ollama','searxng','browser','code-worker','core');Wait-Ollama;Wait-WebServices;Wait-Core;Test-Runtime
  ...
  Invoke-Compose -ComposeArgs @('up','-d','--build','--remove-orphans','core');Wait-Core;Test-Runtime
...
function Start-Runtime([switch]$OpenBrowser){
  ...
  Step 5 $total 'Building isolated Code sandbox' 'User code gets its own worker with no network and no Docker socket.'
  Invoke-Compose -ComposeArgs @('up','-d','--build','code-worker');Wait-CodeWorker
  ...
  Test-Runtime
  Step 9 $total 'Capability safety smoke' 'Web evidence and isolated code execution must both be real before READY.'
  Test-WebInternalSmoke;Test-CodeInternalSmoke
  Step 10 $total 'Ready' 'If this line is green, the vertical slice earned it.'
  Pass 'Personal Agent Rus is ready.'
...
try{
  ...
  switch($Action){
    ...
    'restart'{Require-Docker;Info 'Restarting runtime...';Invoke-Compose -ComposeArgs @('restart','ollama','searxng','browser','code-worker','core');Wait-Ollama;Wait-WebServices;Wait-CodeWorker;Wait-Core;Test-Runtime;Test-CodeInternalSmoke;Pass 'Restart verification passed.'}
    'repair'{Require-Docker;Info 'Repairing runtime without deleting named volumes...';Invoke-Compose -ComposeArgs @('up','-d','--build','--force-recreate','--remove-orphans','ollama','searxng','browser','code-worker','core');Wait-Ollama;Wait-WebServices;Wait-CodeWorker;Ensure-BootstrapModel;Wait-Core;Test-Runtime;Test-CodeInternalSmoke;Pass 'Repair verification passed.'}
    ...
    'verify'{Require-Docker;Wait-Ollama;Wait-WebServices;Wait-CodeWorker;Wait-Core;Test-Runtime;Test-WebInternalSmoke;Test-CodeInternalSmoke}
