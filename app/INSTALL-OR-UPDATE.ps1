...
$ManifestPath=Join-Path $PackageRoot 'product-manifest.json'
if(-not (Test-Path -LiteralPath $ManifestPath)){throw 'product-manifest.json is missing from the release package.'}
try{$Manifest=Get-Content -LiteralPath $ManifestPath -Raw | ConvertFrom-Json}catch{throw 'product-manifest.json is invalid JSON.'}
$Version=([string]$Manifest.version).Trim()
if($Version -notmatch '^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$'){throw "Invalid package version in product-manifest.json: $Version"}
...
try{
  ...
  $launcherMap=[ordered]@{
    ...
    'BACKUP.cmd'='BACKUP.cmd';'UPDATE.cmd'='UPDATE.cmd';
  ...
  # Installed-root PowerShell verifiers must validate the immutable app payload, not stale
  # release-source files that may still exist beside the canonical root after an in-place update.
  foreach($verifyName in @('VERIFY-PACKAGE.ps1','VERIFY-RELEASE.ps1')){
    $proxy=@"
`$ErrorActionPreference='Stop'
`$Verifier=Join-Path `$PSScriptRoot 'app\$verifyName'
if(-not (Test-Path -LiteralPath `$Verifier -PathType Leaf)){
  Write-Host '[FAIL] Installed application verifier is missing: ' `$Verifier -ForegroundColor Red
  exit 1
}
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File `$Verifier
exit `$LASTEXITCODE
"@
    Set-Content -LiteralPath (Join-Path $TargetRoot $verifyName) -Value $proxy -Encoding ASCII
  }
