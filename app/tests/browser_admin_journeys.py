...
def main():
    if sync_playwright is None:
        assert 'providerType' in ADMIN and 'providerUrl' in ADMIN and 'billingSetupChecklist' in ADMIN
        print('PAR_V080_ADMIN_BROWSER_ACCEPTANCE PASS: playwright-unavailable static-contract fallback',flush=True)
        return 0
    exe=chromium_path()
    ...
    with sync_playwright() as p:
        ...
        admin.get_by_role('button',name='Диагностика',exact=True).click();admin.locator('#refreshDiagnostics').click();expect(admin.locator('#adminDiagnostics')).to_contain_text('1.0.0')
        admin.close()
