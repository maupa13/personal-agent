...
def current_gid():
    return str(getattr(os, 'getgid', lambda: 1000)())
...
def base_env(tmp,fake_port,web_port,core_port,token,code_socket,db_name='personal-agent-rus.db',auth_mode='personal'):
    ...
    workspace_root=tmp_root/('workspaces-'+db_name)
    log_dir=tmp_root/'logs'
    code_root=tmp_root/'code-work'
    for path in (tmp_root, db_path.parent, secrets_dir, workspace_root, log_dir, code_root):
    ...
    env=os.environ.copy();env.update({
        ...
        'PA_DB':str(db_path),'PA_SECRETS_DIR':str(secrets_dir),'PA_WORKSPACE_ROOT':str(workspace_root),
        'PA_LOG_DIR':str(log_dir),
...
def main():
    tmp=ROOT/'release-evidence'/'_tmp'/'api-acceptance'/'par-v050-test'
    if tmp.exists(): shutil.rmtree(tmp, ignore_errors=True)
    tmp.mkdir(parents=True, exist_ok=True)
    fake_port,web_port,core_port=free_port(),free_port(),free_port();token='test-admin-token-123'
    ...
    code_socket=pathlib.Path(tmp)/'code-worker.sock';code_env=os.environ.copy();code_env.update({'PYTHONDONTWRITEBYTECODE':'1','PYTHONPATH':str(ROOT)+os.pathsep+code_env.get('PYTHONPATH',''),'PATH':str(fake_bin)+os.pathsep+code_env.get('PATH',''),'PA_CODE_SOCKET':str(code_socket),'PA_CODE_WORK_ROOT':str(pathlib.Path(tmp)/'code-work'),'PA_CODE_RUNNER_UID':current_uid(),'PA_CODE_RUNNER_GID':current_gid(),'PA_CODE_MAX_TIMEOUT_SECONDS':'5'})
    (pathlib.Path(tmp)/'code-work').mkdir(parents=True, exist_ok=True)
    code_worker=None
    if os.name!='nt':
