...
ExperienceService = mod.ExperienceService
ExperienceError = mod.ExperienceError
...
def main() -> int:
    td = ROOT / "release-evidence" / "_tmp" / "alpha4-ux-admin"
    if td.exists():
    ...
    try:
        ...
        svc.init_schema()
        p = svc.preferences("u1")
        assert p["theme"] == "system" and p["execution_policy"] == "auto" and p["tone"] == "normal"
        ...
        svc2 = ExperienceService(db)
        svc2.init_schema()
        p2 = svc2.preferences("u1")
        assert p2["theme"] == "light" and p2["tone"] == "meme"
        ...
        assert '"role": "OWNER"' in MAIN and "local owner admin access requires loopback-only mode" in MAIN
        ...
        for token in ("openai_responses", "/responses", "output_text", "/api/admin/providers", "/api/admin/routing"):
            assert token in MAIN, token
        for token in ("providerType", "providerUrl", "saveRoutes", "OpenAI API", "Responses"):
        ...
        for file in ("local-setup.html", "admin-guide.html"):
            content = (STATIC / file).read_text(encoding="utf-8")
            assert "1.0.0" in content
        ok("GUIDE-A4-001", "local user setup and Admin model/routing guides ship in-product")

        assert "read-only" in APP and "workspace" in APP
        assert "API-ключи" in INDEX
