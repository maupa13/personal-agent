...
if "$Compose=Join-Path $Root 'compose.yaml'" not in lan: errors.append("LAN must use canonical compose.yaml")
if "config\\.env" not in lan: errors.append("LAN must use canonical config/.env in installed app layout")
