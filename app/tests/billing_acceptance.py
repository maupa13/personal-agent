...
def main():
    tmp=ROOT/'release-evidence'/'_tmp'/'billing-acceptance'/'par-billing-test'
    if tmp.exists(): shutil.rmtree(tmp, ignore_errors=True)
    tmp.mkdir(parents=True, exist_ok=True)
    fake_port,web_port,pay_port,core_port=free_port(),free_port(),free_port(),free_port();token='billing-admin-token-123'
