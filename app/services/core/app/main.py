...
from billing_service import BillingService, BillingError, PaymentConfigurationError, InferenceUsage, FREE_THEME_IDS
...
LIST_RESULT_KINDS = {"news", "product", "real_estate", "procurement", "marketplace", "registry"}
...
PUBLIC_SITE_URL = os.getenv("PA_PUBLIC_URL", "").strip().rstrip("/")
SITE_TITLE = os.getenv("PA_SITE_TITLE", "Родной Агент").strip() or "Родной Агент"
SITE_DESCRIPTION = os.getenv(
    "PA_SITE_DESCRIPTION",
    "AI-помощник для чата, файлов, веб-поиска, кода, задач, монетизации и администрирования.",
).strip() or "AI-помощник для чата, файлов, веб-поиска, кода, задач, монетизации и администрирования."
SITE_KEYWORDS = [item.strip() for item in os.getenv(
    "PA_SITE_KEYWORDS",
    "AI помощник, личный ассистент, чат, файлы, веб-поиск, код, задачи, VPS, биллинг, SEO",
).split(",") if item.strip()]
GOOGLE_SITE_VERIFICATION = os.getenv("PA_GOOGLE_SITE_VERIFICATION", "").strip()
YANDEX_VERIFICATION = os.getenv("PA_YANDEX_VERIFICATION", "").strip()
SEO_HEAD_SNIPPETS = os.getenv("PA_HEAD_SNIPPETS", "").strip()
SEO_BODY_SNIPPETS = os.getenv("PA_BODY_SNIPPETS", "").strip()
SEO_CSP_EXTRA = os.getenv("PA_CSP_EXTRA", "").strip()
SEO_PUBLIC_ROUTES = {"/", "/index.html", "/why", "/why.html", "/user-guide", "/user-guide.html", "/terms", "/terms.html", "/privacy", "/privacy.html", "/cookies", "/cookies.html", "/disclaimer", "/disclaimer.html"}
SEO_PRIVATE_ROUTES = {"/admin", "/admin/", "/admin.html", "/account", "/account/", "/account.html", "/login", "/login/", "/login.html", "/register", "/register/", "/register.html"}
SEO_PAGE_META: dict[str, dict[str, str]] = {
    "/": {
        "title": SITE_TITLE,
    ...
    "/why": {
        "title": "Почему " + SITE_TITLE,
        "description": "Кратко о подходе продукта: проверяемый результат, контроль над данными, прозрачная маршрутизация и разделение ролей.",
        "canonical_path": "/why",
    },
    "/user-guide": {
        "title": "Руководство — " + SITE_TITLE,
        "description": "Как пользоваться продуктом: чат, веб-поиск, файлы, код, задачи, приватность и локальная установка.",
        "canonical_path": "/user-guide",
    },
    "/terms": {
        "title": "Пользовательское соглашение — " + SITE_TITLE,
        "description": "Правила использования сервиса, включая роли, данные, оплату и ограничения.",
        "canonical_path": "/terms",
    },
    "/privacy": {
        "title": "Политика конфиденциальности — " + SITE_TITLE,
        "description": "Какие данные обрабатываются, как используются cookies, платежные и технические журналы.",
        "canonical_path": "/privacy",
    },
    "/cookies": {
        "title": "Cookies — " + SITE_TITLE,
        "description": "Что cookies могут хранить и как сервис использует их для входа, настроек, защиты и аналитики.",
        "canonical_path": "/cookies",
    },
    "/disclaimer": {
        "title": "Дисклеймер — " + SITE_TITLE,
        "description": "Ограничения сервиса: он не заменяет профессиональную консультацию и требует проверки критичных решений.",
        "canonical_path": "/disclaimer",
    },
}


def _site_base_url(request_host: str | None = None) -> str:
    if PUBLIC_SITE_URL:
...
def _seo_route(path: str) -> str:
    clean = urllib.parse.urlsplit(path or "").path or "/"
    if clean in {"/index.html", "/"}:
        return "/"
    if clean in {"/why.html", "/why"}:
        return "/why"
    if clean in {"/user-guide.html", "/user-guide"}:
        return "/user-guide"
    if clean in {"/terms.html", "/terms"}:
        return "/terms"
    if clean in {"/privacy.html", "/privacy"}:
        return "/privacy"
    if clean in {"/cookies.html", "/cookies"}:
        return "/cookies"
    if clean in {"/disclaimer.html", "/disclaimer"}:
        return "/disclaimer"
    return clean


def _seo_meta(route: str, request_host: str | None = None) -> dict[str, str]:
    base_url = _site_base_url(request_host).rstrip("/")
    route_key = _seo_route(route)
    meta = dict(SEO_PAGE_META.get(route_key, {}))
    if not meta:
        meta = {
            "title": SITE_TITLE,
            "description": SITE_DESCRIPTION,
            "canonical_path": route_key if route_key.startswith("/") else "/",
        }
    meta.setdefault("title", SITE_TITLE)
    meta.setdefault("description", SITE_DESCRIPTION)
    meta.setdefault("canonical_path", route_key if route_key.startswith("/") else "/")
    canonical_path = meta["canonical_path"] if meta["canonical_path"].startswith("/") else f"/{meta['canonical_path']}"
    meta["canonical"] = base_url + ("/" if canonical_path == "/" else canonical_path)
    meta["robots"] = "noindex,nofollow" if route_key in SEO_PRIVATE_ROUTES or route_key.startswith("/share/") else "index,follow"
    return meta


def _seo_head_markup(route: str, request_host: str | None = None) -> str:
    meta = _seo_meta(route, request_host)
    ...
    keywords = html.escape(", ".join(SITE_KEYWORDS), quote=True) if SITE_KEYWORDS else ""
    parts = [
    ...
    if keywords:
        parts.insert(3, f'<meta name="keywords" content="{keywords}">')
    if GOOGLE_SITE_VERIFICATION:
        parts.append(f'<meta name="google-site-verification" content="{html.escape(GOOGLE_SITE_VERIFICATION, quote=True)}">')
    if YANDEX_VERIFICATION:
        parts.append(f'<meta name="yandex-verification" content="{html.escape(YANDEX_VERIFICATION, quote=True)}">')
    if SEO_HEAD_SNIPPETS:
        parts.append(SEO_HEAD_SNIPPETS)
    return "\n".join(parts) + "\n"


def _seo_body_markup(route: str) -> str:
    if SEO_BODY_SNIPPETS and _seo_route(route) in SEO_PUBLIC_ROUTES:
        return SEO_BODY_SNIPPETS
    return ""


def _robots_txt(request_host: str | None = None) -> str:
    sitemap = _site_base_url(request_host).rstrip("/") + "/sitemap.xml"
    return "\n".join([
        "User-agent: *",
        "Allow: /",
        "Disallow: /api/",
...
def _sitemap_xml(request_host: str | None = None) -> str:
    base = _site_base_url(request_host).rstrip("/")
    today = time.strftime("%Y-%m-%d")
    pages = ["/", "/why", "/user-guide", "/terms", "/privacy", "/cookies", "/disclaimer"]
    rows = [
    ...
    for page in pages:
        rows.append("  <url>")
        rows.append(f"    <loc>{html.escape(base + page)}</loc>")
...
def _metric_host(value: str) -> str:
    parsed = urllib.parse.urlparse(str(value or ""))
    host = (parsed.hostname or parsed.netloc or str(value or "")).strip().lower().strip(".")
    return host or "unknown"


def _parser_bucket(host: str) -> dict[str, Any]:
    with METRICS_LOCK:
        bucket = PARSER_METRICS.setdefault(
            host,
            {
                "requests_total": 0,
                "retry_total": 0,
        ...
        if not isinstance(bucket.get("status_counts"), defaultdict):
            bucket["status_counts"] = defaultdict(int, bucket.get("status_counts") or {})
        return bucket


def parser_request_start(host: str) -> None:
    host = _metric_host(host)
    with METRICS_LOCK:
        bucket = _parser_bucket(host)
        bucket["active_requests"] = int(bucket.get("active_requests", 0)) + 1


def parser_request_finish(host: str, *, status: str = "ok", retry: bool = False, backoff_seconds: float = 0.0) -> None:
    host = _metric_host(host)
    now = time.monotonic()
    with METRICS_LOCK:
        bucket = _parser_bucket(host)
        bucket["active_requests"] = max(0, int(bucket.get("active_requests", 0)) - 1)
        bucket["requests_total"] = int(bucket.get("requests_total", 0)) + 1
        bucket["last_status"] = str(status)
        bucket["last_seen"] = now
        if retry:
            bucket["retry_total"] = int(bucket.get("retry_total", 0)) + 1
        if backoff_seconds:
            bucket["backoff_seconds"] = float(bucket.get("backoff_seconds", 0.0)) + float(backoff_seconds)
        status_counts = bucket.setdefault("status_counts", defaultdict(int))
        status_counts[str(status)] += 1
        recent = bucket.setdefault("recent_hits", [])
        recent.append(now)
        cutoff = now - 60.0
...
def parser_metric_snapshot() -> dict[str, list[dict[str, Any]]]:
    now = time.monotonic()
    with METRICS_LOCK:
        items = []
        for host, bucket in sorted(PARSER_METRICS.items()):
            recent = list(bucket.get("recent_hits") or [])
            rps = round(len([ts for ts in recent if ts >= now - 60.0]) / 60.0, 4)
            items.append({
                "host": host,
                "parser_requests_total": int(bucket.get("requests_total", 0)),
                "parser_429_total": int(bucket.get("status_counts", {}).get("429", 0)),
                "parser_retry_total": int(bucket.get("retry_total", 0)),
                "parser_backoff_seconds": round(float(bucket.get("backoff_seconds", 0.0)), 3),
                "parser_active_requests": int(bucket.get("active_requests", 0)),
                "parser_effective_rps": rps,
                "status_counts": dict(bucket.get("status_counts", {})),
                "last_status": str(bucket.get("last_status") or ""),
                "last_seen": float(bucket.get("last_seen") or 0.0),
            })
    return {"by_host": items}
...
def _settings_json(key: str, default: dict[str, Any]) -> dict[str, Any]:
    raw = setting(key, "")
    if not raw:
...
def vpn_routing_defaults() -> dict[str, Any]:
    ...
    if not allowed_ips and upstream_ip:
        allowed_ips = [upstream_ip]
    return {
        "enabled": _env_bool("PA_VPN_ROUTING_ENABLED", False),
        "mode": "amneziawg" if _env_text("PA_VPN_ROUTING_MODE", "wireguard").lower() == "amneziawg" else "wireguard",
        "preference_id": _env_text("PA_VPN_PREFERENCE_ID", "")[:120],
        "service_role": "vps1",
        "vps1": {
            "name": _env_text("PA_VPN_VPS1_NAME", "VPS1")[:120] or "VPS1",
            "interface": _env_text("PA_VPN_VPS1_INTERFACE", "wg0")[:32] or "wg0",
            "vpn_address": _env_text("PA_VPN_VPS1_ADDRESS", "10.10.0.2/24")[:64] or "10.10.0.2/24",
            "vpn_subnet": _env_text("PA_VPN_VPS1_SUBNET", "10.10.0.0/24")[:64] or "10.10.0.0/24",
            "autostart": _env_bool("PA_VPN_VPS1_AUTOSTART", True),
        },
        "vps2": {
            "name": _env_text("PA_VPN_VPS2_NAME", "VPS2")[:120] or "VPS2",
            "host": _env_text("PA_VPN_VPS2_HOST", "")[:255],
            "vpn_address": _env_text("PA_VPN_VPS2_ADDRESS", "10.10.0.1/24")[:64] or "10.10.0.1/24",
            "endpoint_port": max(1, min(_env_int("PA_VPN_ENDPOINT_PORT", 51820), 65535)),
            "nat_interface": _env_text("PA_VPN_NAT_INTERFACE", "eth0")[:32] or "eth0",
            "ip_forward": _env_bool("PA_VPN_IP_FORWARD", True),
        },
        "upstream": {
            "name": _env_text("PA_VPN_UPSTREAM_NAME", "OpenAPI")[:120] or "OpenAPI",
            "host": _env_text("PA_VPN_UPSTREAM_HOST", "api.example.com")[:255] or "api.example.com",
            "ip": upstream_ip[:255],
            "allowed_ips": allowed_ips,
        },
        "notes": _env_text("PA_VPN_NOTES", "")[:4000],
    }


def vpn_routing_config() -> dict[str, Any]:
    config = _settings_json("vpn_routing_config", vpn_routing_defaults())
    config["enabled"] = bool(config.get("enabled"))
    config["mode"] = "amneziawg" if str(config.get("mode") or "").lower() == "amneziawg" else "wireguard"
    config["preference_id"] = str(config.get("preference_id") or "").strip()[:120]
    config["service_role"] = "vps1"
    config["vps1"] = {
        **vpn_routing_defaults()["vps1"],
        **(config.get("vps1") if isinstance(config.get("vps1"), dict) else {}),
    }
    config["vps1"]["interface"] = str(config["vps1"].get("interface") or "wg0").strip()[:32] or "wg0"
    config["vps1"]["vpn_address"] = str(config["vps1"].get("vpn_address") or "10.10.0.2/24").strip()[:64] or "10.10.0.2/24"
    config["vps1"]["vpn_subnet"] = str(config["vps1"].get("vpn_subnet") or "10.10.0.0/24").strip()[:64] or "10.10.0.0/24"
    config["vps1"]["autostart"] = bool(config["vps1"].get("autostart", True))
    config["vps2"] = {
        **vpn_routing_defaults()["vps2"],
        **(config.get("vps2") if isinstance(config.get("vps2"), dict) else {}),
    }
    config["vps2"]["host"] = str(config["vps2"].get("host") or "").strip()[:255]
    config["vps2"]["vpn_address"] = str(config["vps2"].get("vpn_address") or "10.10.0.1/24").strip()[:64] or "10.10.0.1/24"
    config["vps2"]["endpoint_port"] = max(1, min(int(config["vps2"].get("endpoint_port") or 51820), 65535))
    config["vps2"]["nat_interface"] = str(config["vps2"].get("nat_interface") or "eth0").strip()[:32] or "eth0"
    config["vps2"]["ip_forward"] = bool(config["vps2"].get("ip_forward", True))
    config["upstream"] = {
        **vpn_routing_defaults()["upstream"],
        **(config.get("upstream") if isinstance(config.get("upstream"), dict) else {}),
    }
    config["upstream"]["name"] = str(config["upstream"].get("name") or "OpenAPI").strip()[:120] or "OpenAPI"
    config["upstream"]["host"] = str(config["upstream"].get("host") or "api.example.com").strip()[:255] or "api.example.com"
    config["upstream"]["ip"] = str(config["upstream"].get("ip") or "").strip()[:255]
...
def set_vpn_routing_config(body: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(body, dict):
        raise ValueError("vpn routing config must be an object")
    current = vpn_routing_defaults()
    current["enabled"] = bool(body.get("enabled"))
    current["mode"] = "amneziawg" if str(body.get("mode") or "").lower() == "amneziawg" else "wireguard"
    current["preference_id"] = str(body.get("preference_id") or "").strip()[:120]
    current["vps1"]["interface"] = str((body.get("vps1") or {}).get("interface") or "wg0").strip()[:32] or "wg0"
    current["vps1"]["vpn_address"] = str((body.get("vps1") or {}).get("vpn_address") or "10.10.0.2/24").strip()[:64] or "10.10.0.2/24"
    current["vps1"]["vpn_subnet"] = str((body.get("vps1") or {}).get("vpn_subnet") or "10.10.0.0/24").strip()[:64] or "10.10.0.0/24"
    current["vps1"]["autostart"] = bool((body.get("vps1") or {}).get("autostart", True))
    current["vps2"]["name"] = str((body.get("vps2") or {}).get("name") or "VPS2").strip()[:120] or "VPS2"
    current["vps2"]["host"] = str((body.get("vps2") or {}).get("host") or "").strip()[:255]
    current["vps2"]["vpn_address"] = str((body.get("vps2") or {}).get("vpn_address") or "10.10.0.1/24").strip()[:64] or "10.10.0.1/24"
    current["vps2"]["endpoint_port"] = max(1, min(int((body.get("vps2") or {}).get("endpoint_port") or 51820), 65535))
    current["vps2"]["nat_interface"] = str((body.get("vps2") or {}).get("nat_interface") or "eth0").strip()[:32] or "eth0"
    current["vps2"]["ip_forward"] = bool((body.get("vps2") or {}).get("ip_forward", True))
    current["upstream"]["name"] = str((body.get("upstream") or {}).get("name") or "OpenAPI").strip()[:120] or "OpenAPI"
    current["upstream"]["host"] = str((body.get("upstream") or {}).get("host") or "api.example.com").strip()[:255] or "api.example.com"
    ...
    with DB_LOCK, db() as conn:
        ...
        conn.execute(
            ...
            ("admin.vpn_routing_changed", json.dumps({"enabled": current["enabled"], "mode": current["mode"], "preference_id": current["preference_id"], "upstream": current["upstream"]["host"]}, ensure_ascii=False), now_ts()),
        )
        conn.commit()
    return current


def vpn_routing_plan(config: dict[str, Any] | None = None) -> dict[str, Any]:
    cfg = config or vpn_routing_config()
    vps1 = cfg.get("vps1") if isinstance(cfg.get("vps1"), dict) else {}
    vps2 = cfg.get("vps2") if isinstance(cfg.get("vps2"), dict) else {}
    ...
    vpn_address = str(vps1.get("vpn_address") or "10.10.0.2/24").strip() or "10.10.0.2/24"
    vpn_subnet = str(vps1.get("vpn_subnet") or "10.10.0.0/24").strip() or "10.10.0.0/24"
    nat_interface = str(vps2.get("nat_interface") or "eth0").strip() or "eth0"
    ...
    server_allowed_ips = [str(vps1.get("vpn_address") or "10.10.0.2/24").strip() or "10.10.0.2/24"]
    client_config = "\n".join([
        "[Interface]",
        ...
        f"Endpoint = {vps2_host}:{endpoint_port}" if vps2_host else f"Endpoint = <VPS2_HOST>:{endpoint_port}",
    ...
    server_config = "\n".join([
        "[Interface]",
        f"Address = {vps2_vpn_address}",
        "ListenPort = " + str(endpoint_port),
        "PrivateKey = <VPS2_PRIVATE_KEY>",
        "",
        "[Peer]",
        "PublicKey = <VPS1_PUBLIC_KEY>",
        f"AllowedIPs = {', '.join(server_allowed_ips)}",
        "",
    ])
    verification = {
        "route_check": f"ip route get {upstream_ip or allowed_ips[0].split('/')[0]}",
        "api_check": f'curl https://{upstream_host}/v1/models -H "Authorization: Bearer $API_KEY"',
        "upstream_host": upstream_host,
        "upstream_ip": upstream_ip,
        "preference_id": str(cfg.get("preference_id") or ""),
        "mode": str(cfg.get("mode") or "wireguard"),
        "service_role": str(cfg.get("service_role") or "vps1"),
        "vps1_interface": interface,
        "vps2_interface": vps2_interface,
        "vps2_host": vps2_host,
    ...
    return {
        ...
        "server_commands": [
            ...
            f"sudo iptables -t nat -A POSTROUTING -s {server_allowed_ips[0]} -o {nat_interface} -j MASQUERADE",
        ],
...
def request_json(url: str, payload: dict[str, Any] | None = None, timeout: int = 180, headers: dict[str, str] | None = None, method: str | None = None) -> Any:
    ...
    host = _metric_host(url)
    backoff = 0.25
    last_error: Exception | None = None
    for attempt in range(3):
        parser_request_start(host)
        try:
            with urllib.request.urlopen(req, timeout=timeout) as resp:
                raw = resp.read().decode("utf-8")
                parser_request_finish(host, status=str(getattr(resp, "status", 200)))
                return json.loads(raw) if raw else {}
        except urllib.error.HTTPError as exc:
            last_error = exc
            retryable = exc.code in {429, 502, 503, 504}
            parser_request_finish(host, status=str(exc.code), retry=bool(retryable and attempt < 2), backoff_seconds=backoff if retryable and attempt < 2 else 0.0)
            ...
            last_error = exc
            parser_request_finish(host, status="error")
            raise
    if last_error:
        raise last_error
...
def request_reachable(url: str, timeout: float = 1.5) -> bool:
    ...
    try:
        ...
        parser_request_finish(host, status=str(exc.code))
...
def execute_inference_for_user(user: dict[str, Any], route: dict[str, str], messages: list[dict[str, str]], spec: dict[str, Any], *, source: str) -> tuple[str, dict[str, Any], dict[str, str], str | None, dict[str, Any]]:
    ...
    if not provider:
        raise ApiError(502, "Настроенный AI-провайдер сейчас недоступен")
    allowed, reason = BILLING.route_allowed(user, provider, messages=messages, spec=spec)
...
def _source_kind(question: str, scenario: dict[str, Any] | None = None) -> str:
    ...
    if sid in {"marketplace", "marketplaces"} or category == "marketplace" or any(word in lower for word in ("маркетплейс", "маркетплейсы", "marketplace", "marketplaces")):
        return "marketplace"
    if sid in {"registry", "registries"} or category in {"registry", "registries"} or any(word in lower for word in ("реестр", "реестры", "registry", "registries")):
        return "registry"
...
def public_source_card(source: dict[str, Any], *, kind: str = "source") -> dict[str, Any]:
    ...
    return {
        ...
        "summary": summary,
        "kind": kind if kind in {"news", "product", "real_estate", "procurement", "marketplace", "registry", "source"} else "source",
...
def gather_web_evidence(
    ...
    elif not direct_domains and profile_domains and categories.intersection({"procurement", "real_estate", "marketplace", "registry"}):
    ...
    for url in urls:
        ...
        try:
            ...
            if excerpt:
                ...
                seen.add(canonical)
            parser_request_finish(url, status="200")
        except Exception as exc:
            parser_request_finish(url, status=type(exc).__name__.upper(), retry=True)
        ...
        try:
            ...
            for item in search_web(search_query or query_text, limit=max_sources * 3, category=category):
                ...
                try:
                    ...
                    seen.add(canonical)
                    parser_request_finish(item["url"], status="200")
                except Exception as exc:
                    parser_request_finish(item["url"], status=type(exc).__name__.upper(), retry=True)
                    excerpt = _clean_web_excerpt(item.get("snippet", ""), 1800)
                    if excerpt:
    ...
    if news_request and discovery_domains and current_usable < max_sources:
        ...
        for root_url in roots:
            try:
                ...
                parser_request_finish(root_url, status="200")
            except Exception:
                parser_request_finish(root_url, status="ERROR", retry=True)
                continue
        attempts = 0
        for article_url in list(dict.fromkeys(candidates)):
            ...
            try:
                ...
                parser_request_start(article_url)
                ...
                seen.add(canonical)
                parser_request_finish(article_url, status="200")
            except Exception:
                parser_request_finish(article_url, status="ERROR", retry=True)
                continue
    ...
    if not any(x.get("status") in {"retrieved", "partial"} and x.get("excerpt") for x in sources):
        for url in deferred_root_urls:
            try:
                ...
                parser_request_finish(url, status="ERROR", retry=True)
                continue
...
def _append_verified_materials(text: str, sources: list[dict[str, Any]], *, kind: str) -> str:
    ...
    labels = {
        ...
        "marketplace": "Подтверждённые товары",
        "registry": "Подтверждённые записи",
    }
    ...
    for idx, source in enumerate(usable[:count], 1):
        ...
        excerpt = _clean_web_excerpt(str(source.get("excerpt") or source.get("search_snippet") or ""), 360)
        first = re.split(r"(?<=[.!?])\s+", excerpt, maxsplit=1)[0].strip() if excerpt else ""
...
def observability_snapshot() -> dict[str, Any]:
    ...
    with DB_LOCK, db() as conn:
        ...
        usage_24h = {"events": 0, "tokens": 0, "cost_rub": 0.0, "local_tokens": 0, "remote_tokens": 0, "by_provider": [], "by_model": [], "by_user": []}
        if "usage_events" in table_names:
            usage_24h["events"] = int(conn.execute("SELECT COUNT(*) FROM usage_events WHERE created_at>=?", (usage_24h_since,)).fetchone()[0])
            usage_24h["tokens"] = int(conn.execute("SELECT COALESCE(SUM(total_tokens),0) FROM usage_events WHERE created_at>=?", (usage_24h_since,)).fetchone()[0])
            usage_24h["cost_rub"] = round(float(conn.execute("SELECT COALESCE(SUM(estimated_cost_microrub),0)/1000000.0 FROM usage_events WHERE created_at>=?", (usage_24h_since,)).fetchone()[0] or 0.0), 4)
            usage_24h["local_tokens"] = int(conn.execute("SELECT COALESCE(SUM(total_tokens),0) FROM usage_events WHERE created_at>=? AND billing_class='LOCAL'", (usage_24h_since,)).fetchone()[0])
            usage_24h["remote_tokens"] = int(conn.execute("SELECT COALESCE(SUM(total_tokens),0) FROM usage_events WHERE created_at>=? AND billing_class IN ('BYOK','PLATFORM_REMOTE','PRIVATE_REMOTE')", (usage_24h_since,)).fetchone()[0])
            usage_24h["by_provider"] = [dict(r) for r in conn.execute("SELECT provider_id AS name, COUNT(*) AS events, COALESCE(SUM(total_tokens),0) AS tokens, COALESCE(SUM(estimated_cost_microrub),0)/1000000.0 AS cost_rub FROM usage_events WHERE created_at>=? GROUP BY provider_id ORDER BY tokens DESC LIMIT 5", (usage_24h_since,))]
            usage_24h["by_model"] = [dict(r) for r in conn.execute("SELECT model_id AS name, COUNT(*) AS events, COALESCE(SUM(total_tokens),0) AS tokens, COALESCE(SUM(estimated_cost_microrub),0)/1000000.0 AS cost_rub FROM usage_events WHERE created_at>=? GROUP BY model_id ORDER BY tokens DESC LIMIT 5", (usage_24h_since,))]
            usage_24h["by_user"] = [dict(r) for r in conn.execute("SELECT user_id AS name, COUNT(*) AS events, COALESCE(SUM(total_tokens),0) AS tokens, COALESCE(SUM(estimated_cost_microrub),0)/1000000.0 AS cost_rub FROM usage_events WHERE created_at>=? GROUP BY user_id ORDER BY tokens DESC LIMIT 5", (usage_24h_since,))]
        ...
        business = {
            "users_total": counts["users"],
            "sessions_active": counts["sessions_active"],
            "tasks_active": counts["tasks_active"],
            "tasks_failed": counts["tasks_failed"],
            "artifacts_total": counts["artifacts"],
            "usage_events_total": counts["usage_events"],
            "usage_events_24h": usage_24h["events"],
            "usage_tokens_24h": usage_24h["tokens"],
            "usage_cost_rub_24h": usage_24h["cost_rub"],
            "usage_local_tokens_24h": usage_24h["local_tokens"],
            "usage_remote_tokens_24h": usage_24h["remote_tokens"],
            "balances_total_rub": round(sum(float(row["balance_rub"]) for row in billing_overview.get("balances", [])), 2),
            "balances_users": len(billing_overview.get("balances", [])),
            "theme_purchases_total": len(billing_overview.get("theme_purchases", [])),
            "theme_purchases_rub": round(sum(float(row["price_rub"]) for row in billing_overview.get("theme_purchases", [])), 2),
            "promo_codes_total": len(billing_overview.get("promo_codes", [])),
            "promo_codes_active": len([row for row in billing_overview.get("promo_codes", []) if row.get("active")]),
            "starter_promo_codes": len([row for row in billing_overview.get("promo_codes", []) if row.get("kind") == "starter"]),
            "promo_redemptions_total": int(conn.execute("SELECT COUNT(*) FROM promo_redemptions").fetchone()[0]) if "promo_redemptions" in table_names else 0,
            "topup_requests_pending": int(conn.execute("SELECT COUNT(*) FROM topup_requests WHERE status='PENDING'").fetchone()[0]) if "topup_requests" in table_names else 0,
            "topup_requests_review_required": int(conn.execute("SELECT COUNT(*) FROM topup_requests WHERE status='REVIEW_REQUIRED'").fetchone()[0]) if "topup_requests" in table_names else 0,
            "topup_requests_approved": int(conn.execute("SELECT COUNT(*) FROM topup_requests WHERE status='APPROVED'").fetchone()[0]) if "topup_requests" in table_names else 0,
            "topup_requests_rejected": int(conn.execute("SELECT COUNT(*) FROM topup_requests WHERE status='REJECTED'").fetchone()[0]) if "topup_requests" in table_names else 0,
    ...
    disk_free_threshold = max(0.01, min(_env_float("PA_OBS_DISK_FREE_THRESHOLD", 0.10), 0.50))
    memory_free_threshold = max(0.01, min(_env_float("PA_OBS_MEMORY_FREE_THRESHOLD", 0.10), 0.50))
    if disk_total and disk_free / disk_total < disk_free_threshold:
        alerts.append({"level": "WARN", "code": "DISK_PRESSURE", "message": f"???????? ????? {int(disk_free_threshold * 100)}% ?????"})
    if memory_total and memory_available / memory_total < memory_free_threshold:
        alerts.append({"level": "WARN", "code": "MEMORY_PRESSURE", "message": f"???????? ????? {int(memory_free_threshold * 100)}% RAM"})
    if counts["tasks_failed"]:
        alerts.append({"level": "INFO", "code": "TASK_FAILURES_PRESENT", "message": f"?????????? ?????: {counts['tasks_failed']}"})
    business["recent_alert_count"] = len(alerts)
    components = {
    ...
    parser_metrics = parser_metric_snapshot()
    parser_totals = {
        "requests_total": sum(int(item.get("parser_requests_total", 0)) for item in parser_metrics.get("by_host", [])),
        "429_total": sum(int(item.get("parser_429_total", 0)) for item in parser_metrics.get("by_host", [])),
        "retry_total": sum(int(item.get("parser_retry_total", 0)) for item in parser_metrics.get("by_host", [])),
        "active_requests": sum(int(item.get("parser_active_requests", 0)) for item in parser_metrics.get("by_host", [])),
        "effective_rps": round(sum(float(item.get("parser_effective_rps", 0.0)) for item in parser_metrics.get("by_host", [])), 4),
    }
    return {
...
def run_deployment_job(job_id: str, target_id: str, credential_body: dict[str, Any], action: str) -> None:
    ...
    try:
        ...
        if action == "bootstrap":
            ...
            update_job(job_id, progress=20, message="Загружаю VPN plan")
            plan = vpn_routing_plan()
            if not plan.get("enabled"):
                raise DeploymentError("VPN routing is disabled in admin settings")
            update_job(job_id, progress=40, message="Применяю VPN config на VPS1")
            result = apply_vpn_plan(session, plan, role="vps1")
            result["vpn_plan"] = {
                "enabled": plan.get("enabled"),
                "mode": plan.get("mode"),
                "preference_id": plan.get("preference_id"),
                "upstream": plan.get("upstream"),
                "vps1": plan.get("vps1"),
                "vps2": plan.get("vps2"),
            }
            update_job(job_id, status="completed", progress=100, message="VPN config применён", result=result)
            with DB_LOCK, db() as conn:
                conn.execute("UPDATE deployment_targets SET last_status='PASS',last_message=?,updated_at=? WHERE id=?", (f"vpn apply PASS ({plan.get('mode')})", now_ts(), target_id)); conn.commit()
        elif action == "vpn-apply-server":
            update_job(job_id, progress=20, message="Загружаю VPN plan")
            plan = vpn_routing_plan()
            if not plan.get("enabled"):
                raise DeploymentError("VPN routing is disabled in admin settings")
            update_job(job_id, progress=40, message="Применяю VPN config на VPS2")
            result = apply_vpn_plan(session, plan, role="vps2")
            result["vpn_plan"] = {
                "enabled": plan.get("enabled"),
                "mode": plan.get("mode"),
                "preference_id": plan.get("preference_id"),
            ...
            update_job(job_id, status="completed", progress=100, message="VPN server config применён", result=result)
            with DB_LOCK, db() as conn:
                conn.execute("UPDATE deployment_targets SET last_status='PASS',last_message=?,updated_at=? WHERE id=?", (f"vpn server apply PASS ({plan.get('mode')})", now_ts(), target_id)); conn.commit()
        else:
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def end_headers(self) -> None:
        ...
        csp = "default-src 'self'; style-src 'self'; script-src 'self'; connect-src 'self'; img-src 'self' data:; frame-src 'self'"
        if SEO_CSP_EXTRA:
            csp = f"{csp}; {SEO_CSP_EXTRA}"
    ...
    def _serve_html_with_seo(self, file_path: Path, route: str) -> None:
        raw = file_path.read_text(encoding="utf-8")
        head_markup = _seo_head_markup(route, self.headers.get("Host"))
        if head_markup and "</head>" in raw:
            raw = raw.replace("</head>", head_markup + "</head>", 1)
        body_markup = _seo_body_markup(route)
        if body_markup:
            raw = re.sub(r"(<body[^>]*>)", lambda match: match.group(1) + body_markup, raw, count=1, flags=re.I)
        self._html(200, raw)
    ...
    def do_GET(self) -> None:
        ...
        try:
            if path == "/api/health":
                ...
                if TEST_MODE:
                    self._json(200, {"product": PRODUCT, "version": VERSION, "edition": EDITION, "runtime_profile": RUNTIME_PROFILE, "ready": True, "engine": "ready", "inference": "ready", "web_search": "ready", "browser": "ready", "code": "ready"})
                    return
                engine = False
                model = False
                try:
                ...
                return
            if path == "/robots.txt":
                self._bytes(200, _robots_txt(self.headers.get("Host")).encode("utf-8"), "text/plain; charset=utf-8")
                return
            if path == "/sitemap.xml":
                self._bytes(200, _sitemap_xml(self.headers.get("Host")).encode("utf-8"), "application/xml; charset=utf-8")
                return
            if path == "/api/system":
...
<body>
  <main>
    <h1>{title}</h1>
    <p>Снимок диалога из {PRODUCT}. Ссылка не даёт доступа к аккаунту или workspace.</p>
    <pre>{content}</pre>
  </main>
</body>
</html>"""
            ...
            if path == "/api/billing/me":
                ...
                return
            if path == "/api/billing/promo/redeem":
                user = self._user()
                body = self._body()
                code = str(body.get("code", "")).strip()
                if not code:
                    raise ApiError(400, "promo code required")
                try:
                    result = BILLING.redeem_promo_code(str(user["id"]), code)
                except BillingError as exc:
                    raise ApiError(400, str(exc)) from exc
                log_event("billing.promo_redeemed", user_id=user["id"], promo_code=result["promo_code"], amount_rub=result["amount_rub"], status="SUCCESS")
                self._json(200, {"ok": True, **result})
                return
            if path == "/api/billing/themes/purchase":
                user = self._user()
                body = self._body()
                theme_id = str(body.get("theme_id") or "").strip().lower()
                if not theme_id:
                    raise ApiError(400, "theme_id required")
                try:
                    result = BILLING.purchase_theme(str(user["id"]), theme_id, actor_user_id=str(user["id"]))
                except BillingError as exc:
                    raise ApiError(400, str(exc)) from exc
                if result.get("purchase"):
                    log_event("billing.theme_purchased", user_id=user["id"], theme_id=theme_id, amount_rub=result["purchase"]["price_rub"], status="SUCCESS")
                self._json(200, {"ok": True, **result})
                return
            if path == "/api/billing/topup-requests":
                ...
                if amount_rub is None:
                    raise ApiError(400, "amount_rub required")
                source = str(body.get("source") or "yoomoney")
                note = str(body.get("note") or "")
                payment_reference = str(body.get("payment_reference") or body.get("reference") or "")
                try:
                    request_item = BILLING.create_topup_request(str(user["id"]), amount_rub=amount_rub, source=source, note=note, payment_reference=payment_reference)
                except BillingError as exc:
                    raise ApiError(400, str(exc)) from exc
                log_event("billing.topup_requested", user_id=user["id"], source=source, amount_rub=request_item["amount_rub"], payment_reference=request_item["payment_reference"], status="SUCCESS")
                self._json(201, {"ok": True, "topup_request": request_item})
                return
            ...
            if path == "/api/admin/billing/balance":
                ...
                if not isinstance(body.get("delta_rub"), (int, float, str)):
                    raise ApiError(400, "delta_rub required")
                reason = str(body.get("reason") or "").strip()
                try:
                    result = BILLING.adjust_balance(user_id, delta_rub=body.get("delta_rub"), actor_user_id=str(admin["id"]), source="admin", reason=reason or "admin balance adjustment")
                except BillingError as exc:
                    raise ApiError(400, str(exc)) from exc
                log_event("billing.balance.adjusted", user_id=admin["id"], target_user_id=user_id, delta_rub=body.get("delta_rub"), status="SUCCESS")
                self._json(200, {"ok": True, "balance": result})
                return
            if path.startswith("/api/admin/billing/topup-requests/") and path.endswith("/approve"):
                admin = self._admin()
                request_id = path.strip("/").split("/")[-2]
                try:
                    result = BILLING.approve_topup_request(request_id, reviewer_user_id=str(admin["id"]))
                except BillingError as exc:
                    raise ApiError(400, str(exc)) from exc
                self._json(200, {"ok": True, **result})
                return
            if path.startswith("/api/admin/billing/topup-requests/") and path.endswith("/reconcile"):
                ...
                request_id = path.strip("/").split("/")[-2]
                body = self._body()
                review_note = str(body.get("review_note") or body.get("reason") or body.get("note") or "")
                try:
                    result = BILLING.reconcile_topup_request(request_id, reviewer_user_id=str(admin["id"]), review_note=review_note)
                except BillingError as exc:
                    raise ApiError(400, str(exc)) from exc
                self._json(200, {"ok": True, **result})
                return
            if path.startswith("/api/admin/billing/topup-requests/") and path.endswith("/reject"):
                ...
                request_id = path.strip("/").split("/")[-2]
                body = self._body()
                review_note = str(body.get("review_note") or body.get("reason") or "")
                try:
                    result = BILLING.reject_topup_request(request_id, reviewer_user_id=str(admin["id"]), review_note=review_note)
                except BillingError as exc:
                    raise ApiError(400, str(exc)) from exc
                self._json(200, {"ok": True, **result})
                return
            if path == "/api/admin/billing/promocodes":
                ...
                try:
                    promo = BILLING.create_promo_code(
                        ...
                        code=(str(body["code"]).strip() if body.get("code") else None),
                        description=str(body.get("description") or ""),
                        expires_at=(int(body["expires_at"]) if body.get("expires_at") not in {None, ""} else None),
                        created_by=str(admin["id"]),
                        send_to_email=(str(body.get("send_to_email") or "").strip() or None),
                        send_to_name=(str(body.get("send_to_name") or "").strip() or None),
                        send_subject=(str(body.get("send_subject") or "").strip() or None),
                        send_message=(str(body.get("send_message") or "").strip() or None),
                    )
                except (BillingError, ValueError, TypeError) as exc:
                    raise ApiError(400, str(exc)) from exc
                log_event("billing.promo.created", user_id=admin["id"], promo_code=promo["code"], amount_rub=promo["amount_rub"], email_to=promo.get("email_to"), email_sent=promo.get("email_sent"), status="SUCCESS")
            ...
            if path == "/api/admin/users":
                ...
                users = []
                billing_summary = BILLING.user_billing_summaries([str(row["id"]) for row in rows])
                for row in rows:
                    item = dict(row)
                    try:
                        snapshot = BILLING.snapshot(item)
                        item["billing"] = snapshot["subscription"]
                        item["balance"] = snapshot["balance"]
                    except Exception:
                        item["billing"] = None
                        item["balance"] = None
                    item["billing_summary"] = billing_summary.get(str(item["id"]), {})
                    users.append(item)
            ...
            elif path in {"/terms", "/terms/", "/terms.html"}:
                self.path = "/terms.html"
            elif path in {"/privacy", "/privacy/", "/privacy.html"}:
                self.path = "/privacy.html"
            elif path in {"/cookies", "/cookies/", "/cookies.html"}:
                self.path = "/cookies.html"
            elif path in {"/disclaimer", "/disclaimer/", "/disclaimer.html"}:
                self.path = "/disclaimer.html"
            elif not path.startswith("/static/"):
                raise ApiError(404, "not found")
            target = Path(self.translate_path(self.path))
            if target.suffix.lower() == ".html" and target.is_file():
                self._serve_html_with_seo(target, path)
                return
            return super().do_GET()
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if path == "/api/preferences/experience":
                ...
                try:
                    body = self._body()
                    requested_theme = str(body.get("theme") or EXPERIENCE.preferences(str(user["id"])).get("theme") or "system").strip().lower()
                    if requested_theme not in FREE_THEME_IDS and not BILLING.owns_theme(str(user["id"]), requested_theme):
                        raise ApiError(403, "theme is locked")
            ...
            if path == "/api/admin/vpn-routing":
                ...
                try:
                    ...
                    raise ApiError(400, str(exc)) from exc
                log_event("admin.vpn_routing_changed", user_id=admin.get("id"), enabled=config["enabled"], mode=config["mode"], upstream=config["upstream"]["host"])
