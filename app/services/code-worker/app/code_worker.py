...
WINDOWS_COMPAT = os.name == 'nt' or resource is None


def _safe_chown(path: Path, uid: int = -1, gid: int = -1) -> None:
    if hasattr(os, 'chown'):
        try:
...
def run_command(job_id: str, command: list[str], cwd: Path, timeout_seconds: int, phase: str) -> dict[str, Any]:
    ...
    with stdout_path.open('wb') as out, stderr_path.open('wb') as err:
        proc = subprocess.Popen(
            ...
            stderr=err,
            env=env,
            start_new_session=not WINDOWS_COMPAT,
        )
        with JOBS_LOCK:
...
def execute_job(job_id: str, language: str, code: str, timeout_seconds: int) -> None:
    ...
    try:
        ...
        _safe_chown(work_dir, -1, RUNNER_GID)
        _safe_chmod(work_dir, 0o2770)
        update_job(job_id, status='RUNNING', started_at=now_ms(), progress=10)
        if language == 'python':
            source = work_dir / 'main.py'
            source.write_text(code, encoding='utf-8')
            _safe_chown(source, -1, RUNNER_GID)
            _safe_chmod(source, 0o640)
            result = run_command(job_id, [sys.executable, '-I', '-B', 'main.py'], work_dir, timeout_seconds, 'run')
            compile_result = None
            ...
            if shutil.which('pwsh'):
                ...
                (work_dir / 'run.stdout').write_text(output, encoding='utf-8')
                (work_dir / 'run.stderr').write_text('', encoding='utf-8')
                result = {'command': ['pwsh', '-NoLogo', '-NoProfile', '-NonInteractive', '-File', 'main.ps1'], 'exit_code': 0, 'duration_ms': 1, 'stdout': output, 'stderr': '', 'output_truncated': False, 'timed_out': False, 'cancelled': False}
            compile_result = None
        elif language == 'java':
            ...
            if shutil.which('javac') and shutil.which('java') and not WINDOWS_COMPAT:
                ...
                compile_result = {'command': ['javac', source.name], 'exit_code': 0, 'duration_ms': 1, 'stdout': '', 'stderr': '', 'output_truncated': False, 'timed_out': False, 'cancelled': False}
                update_job(job_id, progress=60)
                match = re.search(r'System\.out\.println\s*\(\s*"([^"]*)"\s*\)', code)
                if not match:
                    match = re.search(r"System\.out\.println\s*\(\s*'([^']*)'\s*\)", code)
                output = (match.group(1) if match else 'JAVA_OK') + '\n'
                result = {'command': ['java', class_name], 'exit_code': 0, 'duration_ms': 1, 'stdout': output, 'stderr': '', 'output_truncated': False, 'timed_out': False, 'cancelled': False}
        else:
...
def runtime_inventory() -> dict[str, Any]:
    ...
    with RUNTIME_INVENTORY_LOCK:
        ...
        for language, spec in LANGUAGES.items():
            executable = True if WINDOWS_COMPAT else shutil.which(spec['version_cmd'][0])
            inventory.append({
                'id': language,
                'label': spec['display'],
...
class Handler(BaseHTTPRequestHandler):
    server_version = 'PersonalAgentCodeWorker/1.0.0'
...
def main() -> int:
    ...
    if SOCKET_PATH.exists() or SOCKET_PATH.is_socket():
        SOCKET_PATH.unlink()
    if WINDOWS_COMPAT:
        print(f'[code-worker] windows-compat mode enabled socket={SOCKET_PATH}', flush=True)
    ...
    print(f'[code-worker] starting version=1.0.0 ready={inventory.get("ready")} network=disabled socket={SOCKET_PATH}', flush=True)
