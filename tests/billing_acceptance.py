...
def main():
    ...
    try:
        ...
        assert {p['id']:p['remote_token_limit'] for p in plans['plans']}=={'LIGHT':0,'MEDIUM':100000,'PRO':500000}

        _,owner,owner_headers=req(base+'/api/auth/register',method='POST',body={'email':'owner@example.test','display_name':'Owner','password':'strong-pass-123'},expect=201)
        _,owner_snap,_=req(base+'/api/billing/me',headers={'Cookie':cookie_from(owner_headers)},expect=200)
        assert owner_snap['subscription']['billing_exempt'] and owner_snap['quota']['platform_remote_tokens_limit'] is None
        ...
        _,me,_=req(base+'/api/auth/me',headers={'Cookie':cookie},expect=200);assert me['user']['role']=='USER'
        ...
        req(base+'/api/admin/billing/plans/MEDIUM',method='POST',body={'remote_token_limit':0,'remote_cost_limit_rub':0},token=token,expect=200)
        req(base+'/api/admin/users/'+me['user']['id']+'/plan',method='POST',body={'plan_id':'MEDIUM'},token=token,expect=200)
        ...
        _,theme,_=req(base+'/api/billing/themes/purchase',method='POST',body={'theme_id':'sunset'},headers={'Cookie':cookie,'X-CSRF-Token':csrf},expect=200);assert theme['owned'] and theme['balance']['balance_rub']==51
        _,owned_again,_=req(base+'/api/billing/themes/purchase',method='POST',body={'theme_id':'sunset'},headers={'Cookie':cookie,'X-CSRF-Token':csrf},expect=200);assert owned_again['already_owned'] and owned_again['balance']['balance_rub']==51
        _,snap,_=req(base+'/api/billing/me',headers={'Cookie':cookie,'X-CSRF-Token':csrf},expect=200);assert 'sunset' in snap['owned_themes'] and any(item['id']=='sunset' and item['owned'] for item in snap['themes'])
        ...
        req(base+'/api/admin/users/'+uid+'/plan',method='POST',body={'plan_id':'MEDIUM'},token=token,expect=200)
