...
evidence_dir = pathlib.Path(os.getenv("PA_EVIDENCE_DIR", root / "release-evidence" / "1.0.0-local"))
evidence_dir.mkdir(parents=True, exist_ok=True)
temp_root = evidence_dir / "_tmp"
...
steps = [
    ...
    (["SCN-001", "SCN-002", "SCN-003", "SCN-004", "SCN-004A", "SCN-005", "SCN-006", "SCN-006A", "SCN-007", "SCN-007A", "SCN-007B", "SCN-007C", "SCN-008", "SCN-009", "SITE-001", "SITE-002", "SITE-003", "SITE-004", "SITE-005", "SITE-006"], "scenario-site-preferences-acceptance", [sys.executable, str(root / "tests" / "scenario_acceptance.py")]),
    ...
    (["REL-A7-001", "PERF-A7-001", "OBS-A7-001"], "alpha7-runtime-timing-acceptance", [sys.executable, str(root / "tests" / "alpha7_runtime_timing_acceptance.py")]),
    (["REL-A8-001", "PERF-A8-001", "PERF-A8-002", "OBS-A8-001", "REG-A8-001"], "alpha8-bootstrap-smoke-acceptance", [sys.executable, str(root / "tests" / "alpha8_bootstrap_smoke_acceptance.py")]),
    (["RESULT-A7-001", "RESULT-A7-002", "NEWS-A7-001", "NEWS-A7-002", "UX-SELECT-A7-001", "UX-LIVE-A7-001"], "alpha7-live-results-acceptance", [sys.executable, str(root / "tests" / "alpha7_live_results_acceptance.py")]),
    (["UX-SELECT-A7-BROWSER", "UX-SCALE-A7-BROWSER", "UX-SCALE-A7-PERSIST", "RESULT-A7-BROWSER-001", "NEWS-A7-BROWSER-001", "UX-LIVE-A7-BROWSER-001", "UX-LIVE-A7-BROWSER-002"], "alpha7-live-results-browser", [sys.executable, str(root / "tests" / "alpha7_live_results_browser.py")]),
...
if not failed_code:
    ...
    for step in steps[3:]:
        if not PLAYWRIGHT_AVAILABLE and ("browser" in step[1] or step[1].endswith("-browser")):
            print(f"=== RELEASE GATE: {step[1]} ===", flush=True)
            note = "[BLOCKED_ENVIRONMENT] playwright is not installed in the current interpreter; browser suites are skipped.\n"
            print(note, end="")
            records.extend([{
                "test_id": test_id,
                "name": step[1],
...
summary = {
    ...
    "environment_bound": [
        ...
        {"test_id": "RESULT-A7-BROWSER-001", "status": "BLOCKED_ENVIRONMENT", "reason": "browser automation library is not installed in the current interpreter"},
        {"test_id": "NEWS-A7-BROWSER-001", "status": "BLOCKED_ENVIRONMENT", "reason": "browser automation library is not installed in the current interpreter"},
        {"test_id": "UX-LIVE-A7-BROWSER-001", "status": "BLOCKED_ENVIRONMENT", "reason": "browser automation library is not installed in the current interpreter"},
        {"test_id": "UX-LIVE-A7-BROWSER-002", "status": "BLOCKED_ENVIRONMENT", "reason": "browser automation library is not installed in the current interpreter"},
        {"test_id": "UX-SELECT-A7-BROWSER", "status": "BLOCKED_ENVIRONMENT", "reason": "browser automation library is not installed in the current interpreter"},
        {"test_id": "ADMIN-A6-BROWSER-001", "status": "BLOCKED_ENVIRONMENT", "reason": "browser automation library is not installed in the current interpreter"},
        {"test_id": "ADMIN-A6-BROWSER-002", "status": "BLOCKED_ENVIRONMENT", "reason": "browser automation library is not installed in the current interpreter"},
    ...
    "deferred_not_applicable_to_current": [
...
print(f"PAR_RELEASE_GATE PASS: static asset-version windows-contract api presets provider-registry auth web-research files-workspace-artifacts code-sandbox accounts-entitlements billing-foundation orchestrator task-engine deployment monitoring lan browser product-shell-v2 productization-conversations-onboarding-rbac-observability scenario-engine-bounded-clarification-site-preferences ux-admin-hardening-execution-policy-share-feedback ux-complete-localization-state-matrix-accessibility search-integrity-result-cards-debug-observability runtime-bootstrap-timing-installer-identity bootstrap-thinking-safe-smoke live-results-min-seven-news-interests-ui-scale-progressive-reveal; evidence={evidence_dir}")
