...
def main():
    ...
    socket_path=tmp/'worker.sock';env=os.environ.copy();env.update({'PYTHONDONTWRITEBYTECODE':'1','PATH':str(bin_dir)+os.pathsep+env.get('PATH',''),'PA_CODE_SOCKET':str(socket_path),'PA_CODE_WORK_ROOT':str(tmp/'work'),'PA_CODE_RUNNER_UID':str(os.getuid()),'PA_CODE_RUNNER_GID':str(os.getgid()),'PA_CODE_MAX_TIMEOUT_SECONDS':'3','PA_CODE_MAX_OUTPUT_BYTES':'2048','PA_CODE_MAX_FILE_BYTES':str(1024*1024),'PA_ADMIN_TOKEN':'MUST_NOT_REACH_CHILD'})
    ...
    try:
        ...
        _,created=request(str(socket_path),'POST','/jobs',{'language':'python','code':'import os\nprint("PY_OK")\nprint(os.environ.get("PA_ADMIN_TOKEN","NO_SECRET"))','timeout_seconds':2});job=await_job(str(socket_path),created['id']);assert job['status']=='COMPLETED' and 'PY_OK' in job['result']['stdout'] and 'NO_SECRET' in job['result']['stdout'] and 'MUST_NOT_REACH_CHILD' not in job['result']['stdout']
        ...
        print('PAR_CODE_WORKER_ACCEPTANCE PASS: python java powershell-contract compile-failure timeout cancel output-limit secret-isolation validation')
        return 0
    finally:
        worker.terminate()
