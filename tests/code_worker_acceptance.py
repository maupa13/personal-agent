...
def main():
    ...
    tmp=TMP_ROOT / f"par-code-worker-{os.getpid()}-{int(time.time()*1000)}";tmp.mkdir(parents=True, exist_ok=True);bin_dir=tmp/'bin';bin_dir.mkdir();pwsh_cmd=bin_dir/'pwsh.cmd';pwsh_stub=bin_dir/'pwsh_stub.py'
    ...
    try:
        ...
        _,created=request(str(socket_path),'POST','/jobs',{'language':'powershell','code':'Write-Output "PS_OK"','timeout_seconds':2});job=await_job(str(socket_path),created['id']);assert job['status']=='COMPLETED' and 'PS_OK' in job['result']['stdout'], job
