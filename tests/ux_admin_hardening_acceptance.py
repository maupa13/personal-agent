...
TMP_ROOT = ROOT / 'release-evidence' / '_tmp' / 'ux-admin-hardening'
...
def main():
    ...
    assert '"role": "OWNER"' in MAIN and 'local_host = host_only in' in MAIN and 'local owner admin access requires loopback-only mode' in MAIN
    ...
    for file in ('local-setup.html','admin-guide.html'):
        content=(STATIC/file).read_text(encoding='utf-8');assert '1.0.0' in content and 'Родной Агент' in content
