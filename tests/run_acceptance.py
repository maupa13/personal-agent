...
TMP_ROOT = ROOT / 'release-evidence' / '_tmp' / 'run-acceptance'
...
def wait_code_health(port, seconds=15):
    end = time.time() + seconds
    last = None
    while time.time() < end:
        try:
            status, body, _ = req(f'http://127.0.0.1:{port}/health', expect=None, timeout=1)
            ...
            last = repr(exc)
        time.sleep(.1)
    raise AssertionError(f'tcp service did not become ready: {last}')
