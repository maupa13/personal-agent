...
TMP_ROOT = ROOT / 'release-evidence' / '_tmp' / 'alpha6-search-debug'
...
def policy_persist():
    with repo_tmp("alpha6") as td:
        svc=ScenarioService(pathlib.Path(td)/'db.sqlite');svc.init_schema()
        ...
        again=ScenarioService(pathlib.Path(td)/'db.sqlite');again.init_schema();value2=again.search_policy()
...
def timing_persist():
    with repo_tmp("alpha6") as td:
        store=ConversationStore(pathlib.Path(td)/'db.sqlite');store.init_schema();c=store.create('u1')
...
def payment_setup():
    with repo_tmp("alpha6") as td:
        svc=BillingService(pathlib.Path(td)/'db.sqlite',pathlib.Path(td)/'secrets',test_mode=True);svc.init_schema()
