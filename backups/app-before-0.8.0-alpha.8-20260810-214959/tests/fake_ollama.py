...
LOCK=threading.Lock()
LAST={"model":None,"messages":None,"request_id":"","correlation_id":"","think":None}
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path=='/v1/responses':
            ...
            with LOCK:
                LAST['model']=body.get('model');LAST['messages']=messages
            self.sendj(200,{'id':'resp_test','object':'response','output_text':'PAR_OPENAI_RESPONSES_OK','usage':{'input_tokens':23,'output_tokens':11,'total_tokens':34}});return
        ...
        if self.path=='/api/chat':
            self.record_trace();body=self.body()
            with LOCK:
                LAST["model"]=body.get('model');LAST["messages"]=body.get('messages');LAST["think"]=body.get('think')
            content='PAR_TEST_OK'
            thinking=''
            # Reproduce the real Qwen3 edge case: with a tiny generation budget,
            # thinking can consume all tokens and final content can be empty.
            # The bootstrap smoke must explicitly disable thinking.
            try:
            ...
            self.sendj(200,{"message":{"role":"assistant","content":content,"thinking":thinking},"used":body.get('model'),"prompt_eval_count":17,"eval_count":8,"total_duration":120000000,"load_duration":20000000,"prompt_eval_duration":30000000,"eval_duration":60000000});return
