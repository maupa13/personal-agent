...
def main():
    ...
    bundle=server_bundle('0.8.0-alpha.3','server-lite','agent.example.test','ADMIN_TOKEN_TEST')
    ...
    result=deploy(session,full,'0.8.0-alpha.3');assert result['hot_verify']=='PASS' and len(session.uploads)==1
    ...
    class FakeResponse:
        ...
        def read(self):return json.dumps({'product':'Personal Agent Rus','version':'0.8.0-alpha.3'}).encode()
    verify=public_hot_verify('agent.example.test','0.8.0-alpha.3',timeout_seconds=5,opener=lambda *_a,**_k:FakeResponse())
