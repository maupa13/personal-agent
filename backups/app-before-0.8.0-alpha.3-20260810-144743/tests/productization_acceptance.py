...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="rpa-productization-") as tmp:
        ...
        log_dir = root / "logs"
        logger = StructuredLogger(log_dir, service="core", version="0.8.0-alpha.3", max_bytes=1024 * 1024, backups=2)
    ...
    check(manifest.get("name") == "Personal Agent Rus" and manifest.get("short_name") == "Personal Agent", "UX-010", "browser application identity manifest is complete")
