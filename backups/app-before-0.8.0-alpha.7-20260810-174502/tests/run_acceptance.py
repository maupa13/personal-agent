...
def main():
    ...
    try:
        ...
        _,_,trace_chat_headers=req(base+'/api/chat',method='POST',body={'mode':'fast','messages':[{'role':'user','content':'trace inference'}]},headers=trace_headers,expect=200)
        ...
        assert bare_scoped['sources'] and all(x.get('domain')=='example.com' and x.get('kind')=='news' for x in bare_scoped['sources']),bare_scoped
        ...
        assert clarify['intent']=='clarification' and clarify['scenario']['id']=='clothing' and clarify['clarification']=={'round':1,'max_rounds':1} and not clarify['sources'] and 'usage' not in clarify,clarify
        ...
        _,web_prefs,_=req(base+'/api/preferences/web',method='POST',body={'search_scope':'selected','prefer_russian':True,'region':'Москва','allowed_domains':['example.com','example.org'],'excluded_domains':['blocked.example']},expect=200);assert web_prefs['preferences']['search_scope']=='selected' and web_prefs['preferences']['region']=='Москва'
        ...
        _,search_policy_saved,_=req(base+'/api/admin/search-policy',method='POST',body={'provider_order':['searxng'],'general_max_sources':4,'news_max_sources':6,'research_max_sources':9,'preferred_domains':['example.com'],'blocked_domains':['blocked.example']},token=token,expect=200)
        assert search_policy_saved['policy']['news_max_sources']==6 and search_policy_saved['policy']['preferred_domains']==['example.com'] and search_policy_saved['policy']['blocked_domains']==['blocked.example'],search_policy_saved
