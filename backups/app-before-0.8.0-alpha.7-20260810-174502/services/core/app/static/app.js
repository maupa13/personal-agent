...
const UI_VERSION='0.8.0-alpha.6';
...
function renderWelcome(){
  ...
  if(state.scenarios.length){
    ...
    for(const raw of state.scenarios.slice(0,8)){
      ...
      card.onclick=()=>{state.scenarioId=item.id;input.value=item.example_prompt||'';resizeInput();renderAll();input.focus();input.setSelectionRange(input.value.length,input.value.length);toast(`${langKey()==='en'?'Assistant':'Помощник'}: ${item.title}`,'success')};gallery.append(card);
    }
    wrap.append(gallery);
  }
...
function renderMessageMeta(message){
  ...
  if(meta.debug&&typeof meta.debug==='object'){const details=document.createElement('details');details.className='message-debug';const summary=node('summary','',L('Диагностика','Diagnostics'));details.append(summary);const rows=[['request',meta.request_id],['correlation',meta.correlation_id],['intent',meta.intent],['routing',formatDuration(meta.routing_ms)],['web',formatDuration(meta.web_ms)],['inference',formatDuration(meta.inference_ms)],['target',meta.debug.execution_target],['execution',meta.debug.execution_policy]];for(const [key,value] of rows){if(value!==undefined&&value!==null&&String(value)!=='')details.append(node('div','message-debug-row',`${key}: ${value}`))}wrap.append(details)}
  return wrap
}
...
function renderChat(){
  ...
  conversation.messages.forEach((message,index)=>{
    ...
    body.append(bubble);const attachments=renderMessageAttachments(message);if(attachments)body.append(attachments);const sources=renderSources(message);if(sources)body.append(sources);const meta=renderMessageMeta(message);if(meta)body.append(meta);body.append(renderMessageActions(message,index));
