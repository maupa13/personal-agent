...
evidence_dir = pathlib.Path(os.getenv("PA_EVIDENCE_DIR", root / "release-evidence" / "0.8.0-alpha.5-local"))
...
summary = {
    ...
    "deferred_not_applicable_to_alpha5": [
...
print(f"PAR_RELEASE_GATE PASS: static asset-version windows-contract api presets provider-registry auth web-research files-workspace-artifacts code-sandbox accounts-entitlements billing-foundation orchestrator task-engine deployment monitoring lan browser product-shell-v2 productization-conversations-onboarding-rbac-observability scenario-engine-bounded-clarification-site-preferences ux-admin-hardening-execution-policy-share-feedback ux-complete-localization-state-matrix-accessibility; evidence={evidence_dir}")
