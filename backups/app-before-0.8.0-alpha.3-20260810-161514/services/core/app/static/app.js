...
function renderSources(message){
  ...
  const wrap=node('div','message-sources');wrap.append(node('div','sources-title',`${L('Источники','Sources')} · ${sources.length}`));const list=node('div','source-list');
...
function prettySize(bytes){const value=Number(bytes)||0;if(value<1024)return `${value} B`;if(value<1024*1024)return `${(value/1024).toFixed(1)} KB`;return `${(value/1024/1024).toFixed(1)} MB`}
...
async function api(path,options){
  ...
  if(!response.ok){const error=new Error(payload.error||'request failed');error.status=response.status;error.code=payload.code;error.capability=payload.capability;throw error}
  return payload;
}
