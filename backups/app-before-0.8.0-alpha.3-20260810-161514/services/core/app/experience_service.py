...
class ExperienceService:
    ...
    def preferences(self, user_id: str) -> dict[str, Any]:
        with self.lock, self._db() as conn:
            row = conn.execute(
                "SELECT ui_language,response_language,theme,execution_policy,tone,updated_at FROM user_experience_preferences WHERE user_id=?",
                (user_id,),
            ).fetchone()
    ...
    def set_preferences(self, user_id: str, body: dict[str, Any]) -> dict[str, Any]:
        ...
        with self.lock, self._db() as conn:
            conn.execute(
                "INSERT INTO user_experience_preferences(user_id,ui_language,response_language,theme,execution_policy,tone,updated_at) VALUES(?,?,?,?,?,?,?) "
                "ON CONFLICT(user_id) DO UPDATE SET ui_language=excluded.ui_language,response_language=excluded.response_language,theme=excluded.theme,execution_policy=excluded.execution_policy,tone=excluded.tone,updated_at=excluded.updated_at",
                (user_id, cleaned["ui_language"], cleaned["response_language"], cleaned["theme"], cleaned["execution_policy"], cleaned["tone"], ts),
            )
            conn.commit()
