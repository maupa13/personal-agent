...
VERSION = os.getenv("PA_VERSION", "0.8.0-alpha.5")
...
def gather_web_evidence(
    ...
    seen: set[str] = set()
    direct_domains = [urllib.parse.urlparse(url).hostname or "" for url in urls]
    direct_domains = [domain.lower().strip(".") for domain in direct_domains if domain]
    ...
    def _admin(self) -> dict[str, Any]:
        ...
        if AUTH_MODE == "personal" and user and str(user.get("id")) == "local-owner":
            ...
            if peer is not None and peer.is_loopback:
                return user
            raise ApiError(403, "local owner admin access is limited to this PC; use accounts mode for LAN administration")
            ...
            if path == "/api/research":
                ...
                try:
                    ...
                    if isinstance(exc, ApiError):
                        raise
                    raise ApiError(502, "AI-провайдер сейчас недоступен") from exc
                payload = {"ok": True, "answer": answer, "sources": [{"title": x.get("title"), "url": x.get("url"), "status": x.get("status"), "strategy": x.get("strategy"), "published_date": x.get("published_date", "")} for x in sources]}
                if BILLING.preference(str(user["id"]))["show_token_usage"]:
                    payload["usage"] = {k: usage_event[k] for k in ("input_tokens", "output_tokens", "total_tokens", "exact", "estimated_cost_rub", "billing_class")}
                if billing_notice:
                    payload["billing_notice"] = billing_notice
            ...
            if path == "/api/chat":
                ...
                if scenario.get("action") == "clarify":
                    ...
                    if conversation_id:
                        try:
                            ...
                            assistant_message = CONVERSATIONS.add_message(str(user["id"]), conversation_id, role="assistant", content=clarification_text)
                        except ConversationError as exc:
                            raise ApiError(409, f"Уточнение подготовлено, но история не сохранена: {exc}") from exc
                    else:
                        assistant_message = {"role": "assistant", "content": clarification_text}
                ...
                if intent:
                    ...
                    try:
                        ...
                        sources = gather_web_evidence(
                            task_text,
                            max_sources=8 if intent == "research" else 5,
                ...
                if expects_russian_reply(raw_messages) and not contains_cyrillic(text):
                    ...
                    try:
                        ...
                        pass
                public_sources = [{"title": src.get("title"), "url": src.get("url"), "status": src.get("status"), "strategy": src.get("strategy"), "published_date": src.get("published_date", "")} for src in sources]
                if conversation_id:
                    try:
                        if persist_user:
                        ...
                        assistant_message = CONVERSATIONS.add_message(str(user["id"]), conversation_id, role="assistant", content=text, sources=public_sources)
                    except ConversationError as exc:
                        raise ApiError(409, f"Ответ получен, но история не сохранена: {exc}") from exc
                else:
                    assistant_message = {"role": "assistant", "content": text}
                duration_ms = int((time.monotonic() - started) * 1000)
                log_event("chat.completed", user_id=user["id"], conversation_id=conversation_id or None, intent=intent or "chat", provider_id=effective_route.get("provider_id"), model_id=effective_route.get("model_id"), duration_ms=duration_ms, status="SUCCESS")
                ...
                payload = {"ok": True, "message": assistant_message, "mode": mode, "preset": preset, "intent": intent or "chat", "sources": public_sources, "conversation_id": conversation_id or None}
