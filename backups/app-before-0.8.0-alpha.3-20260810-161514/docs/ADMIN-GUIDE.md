...
В `0.8.0-alpha.5` Core создаёт/принимает `X-Request-ID` и `X-Correlation-ID`, возвращает их клиенту и передаёт во внутренние Web/Browser/Ollama/Code-worker вызовы. Идентификаторы не добавляются к произвольным внешним web-страницам.
...
С `0.8.0-alpha.5` canonical chat truth находится server-side в SQLite, а не в browser localStorage. Доступ к диалогам/папкам проверяется по `user_id`.
