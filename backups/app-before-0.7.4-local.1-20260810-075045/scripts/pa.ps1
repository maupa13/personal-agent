...
function Ensure-Env {
  $defaults = [ordered]@{
    ...
    'PA_CORE_IMAGE'='personal-agent-core:0.7.3'
    'PA_BROWSER_IMAGE'='personal-agent-browser:0.7.3'
    'PA_SEARXNG_IMAGE'='searxng/searxng:2026.8.5-1689cb1b5'
    'PA_CODE_WORKER_IMAGE'='personal-agent-code-worker:0.7.3'
  ...
  if(Test-Path -LiteralPath $EnvFile){
    ...
    if(([string]$values['PA_CORE_IMAGE']) -match '^(rus-personal-agent-core|personal-agent-core):'){ $values['PA_CORE_IMAGE']='personal-agent-core:0.7.3';$changed=$true }
    if($values.Contains('PA_BROWSER_IMAGE') -and ([string]$values['PA_BROWSER_IMAGE']) -match '^personal-agent-browser:'){ $values['PA_BROWSER_IMAGE']='personal-agent-browser:0.7.3';$changed=$true }
    if($changed){
...
function Test-DeterministicBrowserSecurity([string]$ArtifactsRoot) {
  $bootstrap=Get-EnvValue 'PA_BOOTSTRAP_MODEL';if(-not $bootstrap){$bootstrap='qwen3:0.6b'}
  $coreImage=Get-EnvValue 'PA_CORE_IMAGE';if(-not $coreImage){$coreImage='personal-agent-core:0.7.3'}
  $tests=Join-Path $Root 'tests'
