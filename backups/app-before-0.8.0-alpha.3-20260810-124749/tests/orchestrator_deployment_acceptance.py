...
def main():
    ...
    bundle=server_bundle('0.7.4-local.4','server-lite','agent.example.test','ADMIN_TOKEN_TEST')
    ...
    result=deploy(session,full,'0.7.4-local.4');assert result['hot_verify']=='PASS' and len(session.uploads)==1
    ...
    class FakeResponse:
        ...
        def read(self):return json.dumps({'product':'Personal Agent Rus','version':'0.7.4-local.4'}).encode()
    verify=public_hot_verify('agent.example.test','0.7.4-local.4',timeout_seconds=5,opener=lambda *_a,**_k:FakeResponse())
