...
function Ensure-Env {
  ...
  if(Test-Path -LiteralPath $EnvFile){
    ...
    if(([string]$values['PA_CORE_IMAGE']) -match '^(rus-personal-agent-core|personal-agent-core):'){ $values['PA_CORE_IMAGE']='personal-agent-core:0.8.0-alpha.7';$changed=$true }
    if($values.Contains('PA_BROWSER_IMAGE') -and ([string]$values['PA_BROWSER_IMAGE']) -match '^personal-agent-browser:'){ $values['PA_BROWSER_IMAGE']='personal-agent-browser:0.8.0-alpha.7';$changed=$true }
    if($values.Contains('PA_CODE_WORKER_IMAGE') -and ([string]$values['PA_CODE_WORKER_IMAGE']) -match '^personal-agent-code-worker:'){ $values['PA_CODE_WORKER_IMAGE']='personal-agent-code-worker:0.8.0-alpha.7';$changed=$true }
    if($changed){
...
function Assert-CoreImageCurrent {
  ...
  if(-not $expected){$expected='personal-agent-core:0.8.0-alpha.7'}
...
function Test-Runtime {
  ...
  if(-not $resp.ok -or -not $resp.output_nonempty){Fail 'Bootstrap inference smoke test failed.'}
  $t=$resp.timing
  Info ("INFERENCE bootstrap wall_ms={0} provider_total_ms={1} load_ms={2} prompt_eval_ms={3} generation_ms={4} prompt_tokens={5} output_tokens={6} tokens_per_sec={7}" -f $t.wall_ms,$t.provider_total_ms,$t.load_ms,$t.prompt_eval_ms,$t.generation_ms,$t.prompt_tokens,$t.output_tokens,$t.tokens_per_sec)
...
function Test-DeterministicBrowserSecurity([string]$ArtifactsRoot) {
  $bootstrap=Get-EnvValue 'PA_BOOTSTRAP_MODEL';if(-not $bootstrap){$bootstrap='qwen3:0.6b'}
  $coreImage=Get-EnvValue 'PA_CORE_IMAGE';if(-not $coreImage){$coreImage='personal-agent-core:0.8.0-alpha.7'}
  $tests=Join-Path $Root 'tests'
