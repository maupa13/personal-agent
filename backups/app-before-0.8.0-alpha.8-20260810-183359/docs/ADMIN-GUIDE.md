...
В `0.8.0-alpha.6` Core создаёт/принимает `X-Request-ID` и `X-Correlation-ID`, возвращает их клиенту и передаёт во внутренние Web/Browser/Ollama/Code-worker вызовы. Идентификаторы не добавляются к произвольным внешним web-страницам.
...
С `0.8.0-alpha.6` canonical chat truth находится server-side в SQLite, а не в browser localStorage. Доступ к диалогам/папкам проверяется по `user_id`.
...
## 21. Alpha.4: UX, execution policy и remote test provider

В `personal`-профиле локальный `OWNER` может открыть Admin Console **только с loopback этого ПК** без ввода аварийного токена. Break-glass token остаётся отдельным аварийным механизмом и не заменяет обычную OWNER/ADMIN-аутентификацию в accounts/VPS режиме.

Admin Console управляет **Providers → Models → Routing**. Обычный USER видит продуктовые режимы, а не technical model IDs. Для VPS test profile присутствует provider type **OpenAI API · Responses**; API key хранится и используется только серверной стороной и не должен попадать в browser/logs/diagnostics. Local profile остаётся local-model-first.

Execution Policy пользователя (`local_only`, `prefer_local`, `remote_allowed`, `remote_only`) является backend policy. `local_only` не разрешает скрытый remote fallback.

В Admin доступны Feedback и Share-related operational данные без выдачи raw share token из БД.


## Search Policy / качество источников

`Администрирование → Сайты и поиск` управляет реальными правилами поиска: лимиты источников для обычного поиска/новостей/Research, preferred/blocked domains и site profiles. В alpha.6 рабочим search provider является SearXNG. Yandex/Google не показываются как подключённые до появления реальных adapters.

Явный домен пользователя имеет приоритет над preferred domains. Blocked domain является административной hard-policy.

## ЮKassa — readiness

В `Подписки и Usage → ЮKassa` заполните Shop ID, Secret Key и публичный HTTPS URL. Интерфейс показывает webhook URL и readiness checklist. Заполненная форма сама по себе не означает production-ready: необходимы реально полученный webhook и server-side подтверждённый успешный платёж. Подробности: `PAYMENT-SETUP-YOOKASSA.md`.
## Alpha.7: News/List Result policy

Для `Новости` и `Research` Admin Console не позволяет установить лимит источников ниже 7. Это целевой минимум, а не разрешение дорисовывать результаты: Core возвращает только реально полученный usable evidence. Если источник физически отдаёт меньше, пользователь видит фактическое количество.

Сохранённые пользовательские интересы новостей относятся к USER preferences и не заменяют Admin source policy. При явном домене пользователя strict-domain scope имеет приоритет над общими preferred domains.
